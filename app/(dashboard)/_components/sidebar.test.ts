...
describe('dashboard sidebar cadence navigation (JC-036)', () => {
  ...
  it('places a desktop sidebar collapse control next to the SemuAgent brand', () => {
    expect(sidebarSource).toContain('PanelLeftClose')
    expect(sidebarSource).toContain('사이드바 숨기기')
    expect(sidebarSource).toContain('desktopCollapsed')
    expect(sidebarSource).toContain('onToggleDesktopCollapse')
    expect(layoutSource).toContain('DashboardShell')
