...
않는다. `client`도 사업자 유형과 이름/주소 일부만 보유한다. 사업장 신고정보를 앱에서
입력·확정하는 화면이 후속 범위로 승인되면 청구 도메인과 분리된 **사업장별 신고
profile**을 새 도메인으로 둔다.

제안 이름은 `client_filing_profile`이다. Stage F 첫 runtime은 직원별 급여 기초자료만
표시하고 아래 사업장 신고정보를 입력·표시하지 않으므로 migration을 만들지 않는다.
...
이 값들은 화면을 그릴 때 기존 정본에서 다시 계산한다. Stage E는 새 저장 테이블이나
mutation 없이 기존 급여·직원 read model을 확장하는 것으로 확정했다.
...
- 기본급·기타수당·비과세 식대 등 현재 확정 line에서 분리 가능한 지급 항목. 상여는 별도 정본이 없어 `기타수당`과 구분해 표시하지 않음
- 국민연금·건강보험·장기요양·고용보험
- 기납부 소득세·지방소득세
- 근무기간과 중도퇴사 여부
...
다음처럼 **현재 저장 데이터로 존재를 확인할 수 있는 신호**는 `특례 확인`이며 `급여 준비
완료`로 숨기지 않는다.

- 귀속연도 중도입사: 종전근무지 자료 존재 여부를 홈택스에서 확인
- 귀속연도 중도퇴사: 중도정산 반영 여부를 홈택스에서 확인
- 향후 별도 정본 필드로 확인된 외국인 단일세율·국외근로·특수 감면·기부금·농특세

현재 스키마가 존재 여부를 확인할 수 없는 특례는 `해당 없음`으로 단정하지 않되, 그
부재만으로 모든 직원을 `특례 확인`에 넣지도 않는다.

### 7.1 Stage E Runtime Mapping Decision

완료 연도 직원 행의 화면 상태는 다음 세 종류로 고정한다.

1. `급여 보완`: 월 급여 누락/미마감, line 검토 필요, 명부·입사일 미연결, 급여 항목합계
   불일치. 다른 상태보다 우선하며 급여/직원 명부로 보낸다.
2. `특례 확인`: 급여 자료는 완결됐지만 중도입사·중도퇴사처럼 현재 DB에서 확인되는
   특례 신호가 있다. 홈택스에서 종전근무지·중도정산 반영 여부를 확인한다.
3. `급여 준비 완료`: 위 blocker와 확인 가능한 특례 신호가 없다.

현재 DB에 필드가 없는 외국인 단일세율·국외근로·특수 감면·기부금·농특세는
`해당 없음`으로 단정하지 않는다. 동시에 존재 근거 없이 모든 직원을 `특례 확인`으로
만들지도 않는다. 해당 항목은 홈택스 직접 확인 책임으로만 표시한다. 진행 중 연도는 이
세 상태에 포함하지 않고 페이지 수준 `연도 진행 중`으로 분리한다.

Stage F 최소 데이터는 기존 `payroll_period_summary`, `payroll_employee_line`,
`employee_profile`에 모두 존재한다. 급여 항목·4대보험·소득세·지방소득세의 연간 합계를
같은 tenant·사업장·연도·직원 범위에서 다시 계산한다. 따라서 Stage F에는 신규 schema,
migration, mutation API, AI 호출이 없다. `client_filing_profile`은 별도 사업장 신고정보
화면이 승인될 때만 재검토한다.
...
Stage D HTML Preview는 2026-07-14 프로젝트 오너 승인을 완료했다. 이 승인은 위 §6의
간결한 홈택스 생성 준비 화면에만 적용된다. Stage E는 Screen Flow·UI Design·Pre-Code
Brief와 read model·상태·화면 상태 계약을 동기화했다. Stage F는 이 계약 안에서만 runtime을
교체하며 schema·migration·mutation API를 추가하지 않는다.
...
### 9.2 Stage D Preview Complete · Owner Approved

- [x] [Year-end Settlement Preview](../02_UI_Screens/previews/15_year_end_settlement.html)에 승인 범위를 시각화한다.
- [x] 직원 표의 정보 밀도와 행 상세 구조를 프로젝트 오너에게 확인받았다(2026-07-14).
- [x] 오너 승인 후 Screen Flow·UI Design·Pre-Code Brief를 동기화했다(2026-07-14).
- [x] read model 확장 범위와 `client_filing_profile` migration 보류를 결정했다(2026-07-14).

### 9.3 Stage E Pre-Code Complete

- [x] 승인 Preview의 홈택스 생성 흐름·직원 표·행 상세를 UI 문서에 반영했다.
- [x] tenant·사업장·연도·직원 scope와 급여/보험/기납부세액 최소 필드를 고정했다.
