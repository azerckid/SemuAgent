...
- Implementation Preconditions (착수 전 영향 감사 필수):
  ...
  - [x] 메일 영향 — [Audit §5](../03_Technical_Specs/20_LEGACY_UPLOAD_EMAIL_RETIREMENT_AUDIT.md): 현행 internal_reminder·work-email은 유지, `outbound_email` 기반 고객 요청메일은 은퇴 후보. Slice 2a(2026-07-05)에서 레거시 고객 요청메일 쓰기 API를 410 Gone으로 차단.
...
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1(Route Quarantine) + Slice 1b(죽은 컴포넌트/lib 삭제 + 포털 전용 API 4개 410 차단) + Slice 1c(`session.ts` 포털 조회 dead code 제거) + Slice 2a(레거시 고객 요청메일 쓰기 API 12개 410 차단) 완료. `upload_session`은 현행 source lineage FK라 즉시 삭제 금지 원칙 유지. 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password), direct-upload, work-email, internal reminder는 계속 미변경. `lib/session.ts`는 토큰 검증(`verifyToken`)만 남기고, 포털 렌더링용 `getPortalData`와 하위 헬퍼는 제거. 다음: Slice 2b(`lib/email/missing-request`와 pipeline 내부 보충요청 초안 side effect 감사·제거), 이후 transaction-purpose/outbound_email FK 재설계.
