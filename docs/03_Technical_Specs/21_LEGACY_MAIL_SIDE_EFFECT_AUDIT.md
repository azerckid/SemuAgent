...
- `app/(dashboard)/dashboard/clients/[id]/events/[eventId]/page.tsx` — live 사업장 이벤트 상세 화면의 발송 기록 read가 남아 있음. 요청 메일 발송 UX와 결합되어 있어 별도 slice에서 UI/route 정책을 먼저 확정 후 정리.
...
3. **Slice 2b-3 — Legacy mail UI/read cleanup**
   ...
   - **후속 분리:** live 사업장 이벤트 상세의 발송 기록 read는 요청 메일 발송 UX와 함께 별도 정책 결정 후 정리.
