...
- Implementation Preconditions (착수 전 영향 감사 필수):
  ...
  - [x] Slice 4-0 allowlist 감사 — `upload_session` 117 TS/TSX·`outbound_email` 12 TS/TSX 참조를 카테고리 A~T로 분류, table rebuild 5-phase 전략·4-1 dead code 후보(`createSessionAndSend`, `missing-request` 모듈) 문서화. [Allowlist](../03_Technical_Specs/25_SLICE4_SCHEMA_RETIREMENT_ALLOWLIST.md).
- Acceptance Criteria:
...
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1~2c + Slice 3a~3b + Slice 3c-0~3c-5 + **Slice 4-0(2026-07-06, allowlist 감사)** 완료. 다음: **Slice 4-1** dead code 제거 후보 검토.
