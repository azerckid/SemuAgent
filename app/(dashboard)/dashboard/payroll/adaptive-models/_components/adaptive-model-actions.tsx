...
export function AdaptiveModelActions({
  ...
  const handleApprove = async () => {
    setLoading(true)
    const result = await postAction(`/api/payroll/adaptive-models/${modelId}/approve`)
    setLoading(false)
    if (!result.ok) {
      toast.error(result.error ?? '승인하지 못했습니다')
      return
    }
    toast.success('모델을 승인했습니다. 다음 추출부터 같은 형식의 워크북에 자동 재사용됩니다.')
    router.refresh()
  }

  const handleReject = async () => {
    setLoading(true)
    const result = await postAction(`/api/payroll/adaptive-models/${modelId}/reject`)
    setLoading(false)
    if (!result.ok) {
  ...
  const handleRetire = async () => {
    setLoading(true)
    const result = await postAction(`/api/payroll/adaptive-models/${modelId}/retire`)
    setLoading(false)
    if (!result.ok) {
          ...
          <Sheet>
            ...
            <SheetContent side="right">
              <SheetHeader>
                ...
                <SheetDescription>
                  승인하면 같은 형식의 급여 워크북을 처리할 때 이 모델이 자동으로 재사용됩니다.
                  적용된 row는 항상 검토 대상(needs_review)으로 표시되며, 결과 엑셀은 검토 후에만 생성할 수 있습니다.
                </SheetDescription>
              </SheetHeader>
      ...
      <Sheet>
        ...
        <SheetContent side="right">
          <SheetHeader>
            ...
            <SheetDescription>
              폐기하면 이 모델은 더 이상 자동으로 재사용되지 않습니다. 이미 만들어진 급여 row에는 영향을 주지 않습니다.
            </SheetDescription>
          </SheetHeader>
          <div className="px-4">
            <p className="text-sm font-medium text-foreground">{modelName}</p>
