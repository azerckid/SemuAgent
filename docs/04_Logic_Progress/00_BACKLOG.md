...
| JC-030 | todo | 전자신고 검증 및 파일 생성 (Validation / Path 1a·1b) | `lib/efiling-*`, JC-024·013 | **최우선 — Path 1 세목 확대.** 자료대조 Phase 2 완료. **간이지급명세서·원천세·부가세는 Path 1b 직접입력 정리 화면 구현·검증 완료.** 지방소득세 특별징수는 Path 1a 후보지만 로그인 위택스 원본 입수까지 외부 대기다. 근로소득 지급명세서는 Stage B 법정 필드·canonical 공백 매핑을 완료했으며, 현재 데이터만으로 완전 신고서를 만들 수 없어 UI-First Gate는 NO-GO다. **다음 실행 작업은 Stage C 신뢰 가능한 연말정산 결과 source·신고 사업자 profile·PII ownership 계약**이며, 이를 통과하기 전 화면을 만들지 않는다. 사업장현황신고는 면세 개인사업자 조건부 후순위다. Path 3 암호화·회계파일은 범위 밖. [Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md) · [Annual Wage Stage A Audit](../03_Technical_Specs/55_JC030_ANNUAL_WAGE_STATEMENT_STAGE_A_AUDIT.md) · [Annual Wage Stage B Mapping](../03_Technical_Specs/56_JC030_ANNUAL_WAGE_STATEMENT_FIELD_MAPPING.md) |
...
- Implementation Preconditions (조사·설계 과제):
  ...
  - [ ] **근로소득 지급명세서 Stage C canonical source 계약** — 신뢰 가능한 확정 연말정산 결과 import/확인 source, 신고 사업자 profile 정본, 주민등록번호·부양가족·공제증빙 PII ownership과 보존 경계를 결정한다. 완전 세액 계산 엔진 신설은 별도 법무·세무 검토 epic 없이는 포함하지 않는다
  - [ ] **근로소득 지급명세서 Path 1b UI~Closeout** — Stage C 통과 후 Preview·오너 승인·Pre-Code·read model·browser/QA·문서 closeout
...
- Document Sync Check Update (2026-07-14): **근로소득 지급명세서 Stage B Field Mapping을 완료하고 UI-First Gate를 NO-GO로 판정했다.** 현행 소득세법 시행규칙 별지 제24호서식(1)의 사업자·근무처·급여/비과세·세액·소득공제·세액공제·결정세액·기납부세액·차감징수세액 필드를 전부 ownership별로 분류했다. 현재 DB의 `annualGrossPayKrw`·`annualWithholdingTaxKrw`와 월 급여 라인만으로는 완전 신고서 값을 만들 수 없고, billing profile을 신고 정본으로 재사용하거나 공제·결정세액을 역산하지 않는다. 다음은 Stage C 신뢰 가능한 확정 연말정산 결과 source·신고 사업자 profile·PII 경계 계약이며, 이를 통과하기 전 Preview/runtime은 만들지 않는다.
