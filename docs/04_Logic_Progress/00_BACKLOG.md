...
| JC-004 | doing | Audit copied routes and rename accounting-firm assumptions | `app`, `lib`, `components` | Company self-use terminology and responsibility boundary are reflected in visible routes. 노출 표면 정리(설정 GIWA 'CC 참조메일' 탭 제거) + dead GIWA 컴포넌트 삭제 완료. 레거시 GIWA 워크플로 라우트 6종(sessions·reviews·emails·calendar·checklists·law-search) 및 사업장 하위 GIWA 요청 라우트(events·schedules·request-templates·payroll-requests) redirect 차단 완료. `clients`(=사업장 등록·관리, v1 필수)·`billing`(=요금제)은 기능 유지(차단 제외). clients 화면 용어 사업장화(고객사→사업장) 완료. 설정 업무메일 탭 정리 완료(work-email 패널 '사무소'→'회사', GIWA 고객 리마인더-days 섹션 제거 — 내부 리마인드는 JC-016 담당, reminderDaysBefore/cron은 dormant 유지). 사업장 상세(clients/[id]) GIWA 탭 제거 완료(최근 요청·참조 그룹 → 사업장 문서·사내급여기준·법적기준만 유지, page.tsx 쿼리·고아 컴포넌트 정리). 후속: jaryo-admin(플랫폼 운영) 별도 판단 |
