...
- Implementation Preconditions (조사·설계 과제):
  ...
  - [ ] **근로소득 지급명세서 Stage D Preview** — 간결한 기초자료 준비 화면, 홈택스 생성 흐름, 직원별 `급여 준비 완료/급여 보완/특례 확인` 상태를 HTML Preview로 만들고 오너 승인을 받는다. 승인 전 runtime·schema·API 금지
