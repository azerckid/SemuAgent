...
| JC-030 | todo | 전자신고 검증 및 파일 생성 (Validation / Path 1a·1b) | `lib/efiling-*`, JC-024·013 | **최우선 — Path 1 세목 확대.** 자료대조 Phase 2 완료. **간이지급명세서·원천세·부가세는 Path 1b 직접입력 정리 화면 구현·검증 완료.** 지방소득세 특별징수는 Path 1a 후보지만 로그인 위택스 원본 입수까지 외부 대기다. **다음 실행 작업은 근로소득 지급명세서 Path 1b Stage B 필드·canonical 데이터 공백 매핑**이며, 화면을 먼저 만들지 않는다. 사업장현황신고는 면세 개인사업자 조건부 후순위다. Path 3 암호화·회계파일은 범위 밖. [Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md) · [Annual Wage Stage A Audit](../03_Technical_Specs/55_JC030_ANNUAL_WAGE_STATEMENT_STAGE_A_AUDIT.md) |
...
- Implementation Preconditions (조사·설계 과제):
  ...
  - [ ] **근로소득 지급명세서 Stage B Field Mapping** — 법정/홈택스 직접작성 전체 필드를 canonical source·사용자 입력·홈택스 확인·v1 제외로 분류. 연간 지급액·기납부세액만으로 공제·과세표준·결정세액·환급/추징을 추정하지 않으며 Stage B 전 Preview/runtime 금지
  - [ ] **근로소득 지급명세서 Path 1b UI~Closeout** — Stage B 통과 후 Preview·오너 승인·Pre-Code·read model·browser/QA·문서 closeout
