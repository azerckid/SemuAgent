...
이 문서는 구현 전 계약이다. 실제 migration, seed 함수, 삭제 API, UI 배선은 후속 구현 PR에서 수행한다.
...
6. 삭제 완료 후 dataset은 `deleted`로 표시한다. 물리 dataset row는 감사용으로 보존한다.

## 5. Seed Contract: Preview Data Mapping

| 화면 | Preview 기준 | Seed 핵심 값 |
|:---|:---|:---|
| 회사 홈 | `00_company_home.html` | 샘플컴퍼니(주), `2026-H1`, 미수집 1/24, 분류 대기 18/342, 부가세 공제 검토 3, 급여 확인 필요 1 |
...
| 급여 | `04_payroll.html` | 2026-06, 대상 12명, 지급 42,600,000, 공제 5,840,000, 실지급 36,760,000, 확인 필요 1 |
| 신고지원 | `05_filing_support.html` | 부가세 패키지 잠금, 원천세 준비됨, 4대보험 확인 필요, 접수증 보관 대기 |
| 직원 명부 | `06_employee_directory.html` | 전체 14명, 재직 12, 휴직 1, 퇴사 2, 급여 대상 11, 4대보험 확인 필요 1 |
...
- 구현 PR에서 sample deletion whitelist와 delete order를 실제 FK 관계에 맞춰 확정한다.
- row-level sample badge는 v1 필수는 아니며, 전역 banner와 주요 표 heading badge부터 시작한다.
- preview 수치와 구현 read model 수치가 어긋나면 seed fixture를 조정하고 Preview 계약을 깨지 않는다.
