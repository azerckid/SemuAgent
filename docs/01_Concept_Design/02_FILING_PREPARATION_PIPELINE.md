...
- **Path 1 (현재·베타):** 양식이 있으면 SemuAgent가 공식 비암호화 업로드 파일을 만들고(1a), 없으면 확정 데이터를 `항목 = 값`으로 화면에 정리한다(1b). 사용자가 값을 확인한 뒤 홈택스에서 직접 업로드하거나 직접 입력·제출한다. 양식이 없다는 이유로 세목을 `blocked`로 두지 않는다.
...
| 부가세 | 매출 · 매입 exact VAT fact + 증빙 + 사용자 확정 이력 | 매입/매출세액 집계 + AI 판단 근거·필요 증빙 + 사용자 확정 | JC-035 판단 보조 완료(`done`); Path 1b 직접입력 정리로 제공, 1a 업로드 양식은 Stage A 확인 시 |
