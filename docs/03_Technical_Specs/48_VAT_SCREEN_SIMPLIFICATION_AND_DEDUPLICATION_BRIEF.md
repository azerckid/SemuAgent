...
예외가 0건이면 긴 판단표를 보여주지 않고 정상 자동 정리 요약과 신고 준비 상태만 보여준다.
예외가 있으면 **확인 필요 N건**을 우선 표시하고 정상 건은 접힌 상태를 유지한다.
...
## 5. Candidate Decisions for Owner Review
...
| 정상 거래 표시 | 정상 자동 정리 N건으로 접기 | 기간 합계 확인만으로 충분한 deterministic 자격 조건은 무엇인가? |
| 예외 작업대 | 확인 필요 건만 기본 노출 | 영세율·면세·불공제·안분·누락·취소·중복·불일치가 모두 포함되는가? |
...
- [ ] 명확한 정상 거래는 정상 자동 정리 건수·합계로 접고 거래별 반복 확정을 요구하지 않는다.
- [ ] 영세율·면세·불공제·안분·누락·취소·중복·금액 불일치 등 예외만 기본 작업대에 펼친다.
- [ ] 예외 행은 단독 `확인 필요` 대신 잠정 결론·실제 근거·홈택스 권장 행동을 먼저 표시한다.
- [ ] 담당자 이관은 JC-039 Human Handoff Gate를 통과한 행에만 표시한다.
- [ ] 예외 0건이면 긴 판단표 대신 정상 반영 요약과 신고 준비 상태를 보여준다.
- [ ] 자동 정리는 AI 단독 판단으로 만들지 않고 전자증빙·정확한 금액·deterministic rule을 요구한다.
- [ ] 사용자는 기간 단위 정상 반영 요약과 최종 신고 내용을 명시적으로 확인한다.
