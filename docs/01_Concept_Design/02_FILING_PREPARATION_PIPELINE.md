...
- **Path 1 (현재·베타):** SemuAgent가 홈택스가 직접 수용하는 공식 비암호화 업로드 양식·파일을 만들고, 사용자가 값을 확인한 뒤 홈택스에서 직접 업로드·제출한다.
...
- SemuAgent는 Path 1의 공식 비암호화 업로드 양식·파일, 사전검증, 업로드 안내를 제공한다.
...
| 부가세 | 매출 · 매입 exact VAT fact + 증빙 + 사용자 확정 이력 | 매입/매출세액 집계 + AI 판단 근거·필요 증빙 + 사용자 확정 | JC-035 판단 보조 완료(`done`); Path 1 파일은 Stage A 외부 확인 대기 |
