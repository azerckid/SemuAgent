...
JC-010의 회사 기장검토 분류 큐는 승인된 [02_bookkeeping_review.html](../02_UI_Screens/previews/02_bookkeeping_review.html) 구조를 그대로 따른다. 2026-07-08 승인된 [12_reconciliation_ledger.html](../02_UI_Screens/previews/12_reconciliation_ledger.html)은 Path 1 자료대조원장 전용 Preview이다. 1차 앱 구현은 `/dashboard/bookkeeping/reconciliation-ledger` read-only route로 Preview 계약을 반영하고, 자동 대조 엔진·신규 DB는 별도 구현 Brief가 필요하다. 아래 두 규칙을 강제한다(정적 테스트로 검증):
...
| 사이드바·링크 | `ROUTES.bookkeeping`(`lib/company-home/summary.ts`)은 `/dashboard/bookkeeping` 기장검토 분류 큐로 연결한다. 자료대조원장 하위 메뉴와 회사 홈 CTA는 `/dashboard/bookkeeping/reconciliation-ledger`로 연결한다. |
