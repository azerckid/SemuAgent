...
- **근로소득 지급명세서 신고 산출물은 JC-030**. Stage A는 공식 비암호화 업로드
  ...
  매핑을 완료했다. JC-024는 계속 데이터셋·검토 상태까지만 담당하며 현재 연말정산
  화면을 홈택스 직접입력 화면으로 간주하지 않는다. 전체 정산 결과 정본이 없어
  UI-First Gate는 NO-GO이며 다음은 Stage C source 계약이다([56](./56_JC030_ANNUAL_WAGE_STATEMENT_FIELD_MAPPING.md)).
