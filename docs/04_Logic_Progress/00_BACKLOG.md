...
| JC-008 | done | Review residual npm audit findings | `package.json`, parser/import libraries | `npm audit` 0건 달성. `xlsx`→SheetJS 공식 CDN 0.20.3 핀(prototype pollution·ReDoS 수정, API 동일). `ws`^8.21.0·`postcss`^8.5.10·`esbuild`^0.25.0 npm overrides로 transitive(viem/next/drizzle-kit) 취약점 정리. viem은 http 전송만 사용해 ws DoS 미도달, esbuild override 후 drizzle-kit 정상 검증 |
