...
> Status: VAI-8a implemented; VAI-8b~8e pending
...
- `vat_tax_treatment_ai_result.result_payload_json`은 payload v2와 prompt
  `vat-tax-treatment-v2`를 사용한다. v1 저장 결과는 버전 불일치로 자동 재사용되지 않는다.
...
| 작업 단위 | 내용 | 완료선 |
|:---|:---|:---|
| **VAI-8a · Judgment/Workflow Contract** | 결과 enum·Zod·API·UI 용어 분리 | **완료(2026-07-13)** · provider 결론에서 needs_review 제거, payload/prompt v2, SQL migration 없음 |
