...
다음 제품 작업: 근로소득 지급명세서 Stage D — 간결한 기초자료 준비 HTML Preview·오너 승인
...
| 4 | **근로소득 지급명세서** | JC-024 연간 지급·기납부 원천세 준비 + Stage B 전체 필드 공백 감사 | Stage C 완료 — 홈택스 생성 결과가 최종 정본, SemuAgent는 확정 급여 기초자료 준비 | 업로드 파일·직접입력 화면 없음 | **Path 1b 대상 · Stage D Preview 다음** |
...
4. **Annual Stage D — 다음**: 간결한 기초자료 준비 HTML Preview와 오너 승인.
5. **Annual E 이후 — 대기**: 승인 Preview 기준 Pre-Code·read model·runtime·QA.
