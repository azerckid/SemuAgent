...
판정: 현재 DB만으로 완전한 지급명세서 값을 만들 수 없음 → UI-First Gate NO-GO
다음: Annual Stage C — 신뢰 가능한 연말정산 결과 정본의 취득·확정 계약
...
### 6.2 UI-First Gate: NO-GO

현재 DB에는 완전한 연말정산 결과가 없으므로 근로소득 지급명세서 Preview/runtime을
만들지 않는다. 지금 화면을 만들면 대부분의 핵심 칸이 비거나, 총지급액과 기납부세액에서
세액을 역산하는 잘못을 피할 수 없다.

Path 1b 판정은 유지한다. 세목이 `blocked`된 것이 아니라 **구현 선행 정본이 없는 상태**다.
Stage C가 정본 취득 방식을 확정하면 다시 UI-First Gate를 연다.

## 7. Annual Stage C — Next Work

Stage C는 코드를 먼저 만드는 작업이 아니라, 연말정산 결과의 신뢰 source를 결정하는
작업이다.

1. **우선 검토:** 신뢰 가능한 급여/연말정산 시스템의 확정 결과 또는 공식 결과 문서에서
   제24호서식 필드를 구조화해 가져오는 read-only import.
2. **공용 신고 사업장 profile:** 상호·대표자·사업자등록번호·주소·사업자단위과세 정보를
   청구 profile과 분리해 어느 도메인이 소유할지 결정.
3. **PII 경계:** 주민/외국인등록번호와 가족·계좌정보는 서버 영구 저장 없이 홈택스 직접
   입력 또는 일회성 세션으로만 다룰지 재확인.
4. **정합성:** imported result의 귀속연도·사업장·직원·총급여·기납부세액이 현재 급여
   정본과 맞지 않으면 전체 행을 blocker로 둔다.
5. **대안 결정:** 신뢰 source를 확보할 수 없으면 전체 연말정산 계산 엔진을 별도 법무·세무
   검토 후 새 epic으로 승인하거나, 현 준비·검토 화면까지만 v1 범위로 유지한다.

전체 연말정산 계산 엔진은 이 문서로 승인되지 않는다. 단순 공식으로 공제·세액을 만들거나
AI에게 숫자를 생성하게 하지 않는다.
...
- [ ] 공용 신고 사업장 profile ownership을 결정한다.
- [ ] PII를 저장하지 않는 직접입력/일회성 처리 경계를 재확인한다.
- [ ] 대표 fixture에서 제24호서식 필드가 빠짐없이 채워지는지 검증한다.
- [ ] Stage C 통과 후 HTML Preview와 프로젝트 오너 승인을 요청한다.
