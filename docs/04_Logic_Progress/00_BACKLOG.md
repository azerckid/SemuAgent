...
- Implementation Preconditions (착수 전 영향 감사 필수):
  ...
  - [x] Slice 4-2-0 준비 — [Brief 26](../03_Technical_Specs/26_UPLOAD_SESSION_COLUMN_RETIREMENT_PRE_CODE_BRIEF.md): `upload_session` 컬럼별 retirement 분류 완료. 즉시 table rebuild 금지(`token_hash/upload_url/request_kind/request_event_id/request_email_*`·AI/review context 사용처 잔존), 다음은 4-2a legacy session/request context surface 정리 결정.
- Acceptance Criteria:
...
- Document Sync Check: Slice 4-0 allowlist + **Slice 4-1(2026-07-06, dead code 제거)** + **Slice 4-2-0(2026-07-06, 컬럼 retirement brief)** 완료. 다음: **Slice 4-2a** legacy session/request context surface 정리 결정 및 구현. 실제 `upload_session` table rebuild는 삭제 대상 컬럼 runtime read/write 0 확인 전까지 금지.
