...
다음 제품 작업: 로그인 위택스에서 공식 Excel 원본 입수·hash/시트/열 고정 → Local Income Stage B
파일 트랙 다음(1a): 지방소득세 공식 원본 입수 후 B~G
이후: 사업장현황신고 → 연말 지급명세서 (각 세목 1a 없으면 1b)
...
### 4.5 Following Tax Tracks

지방소득세 특별징수 다음 세목은 동일한 A~G 순서로 진행한다.

1. **Business Status A~G** — 면세 개인만 허용하고 비대상 사업자는 차단한다.
2. **Annual Statement A~G** — 간이지급 파일 레이아웃을 재사용한다고 가정하지 않는다.
