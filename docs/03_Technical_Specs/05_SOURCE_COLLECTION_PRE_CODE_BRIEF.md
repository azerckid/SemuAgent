...
| Client UI | `staff-direct-upload-workspace.tsx`를 승인 Preview 구조로 점진 교체. 신규 `source-collection.tsx` 컴포넌트 분리 |
...
직접 업로드" 문구 중심이다. JC-009는 이를 **수집 완결성 → 업로드 → 자료유형 타일 →
수집 상태 표 → 미수집·확인 필요** 순서의 회사용 화면으로 바꾼다.
...
| UI | `staff-direct-upload-workspace.tsx` → `source-collection.tsx` | Preview 구조로 교체 |
...
5. `staff-direct-upload-workspace.tsx`에서 회사용으로 불필요한 GIWA UI 제거·위임.
