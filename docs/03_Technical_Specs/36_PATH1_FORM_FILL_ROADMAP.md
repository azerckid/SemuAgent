...
다음 제품 작업: 근로소득 지급명세서 Stage F — runtime 실제 DB 연결·단위/브라우저 QA·문서 closeout
...
| 4 | **근로소득 지급명세서** | JC-024 월 확정 급여·직원 명부 + Stage B 전체 필드 공백 감사 | Stage C 정본/PII, Stage D 승인 Preview, Stage E Pre-Code 완료 | runtime 미연결 · 신규 파일/DB 없음 | **Path 1b 대상 · Stage F runtime 다음** |
...
현재 runtime 연말정산 read model은 연간 지급합계·기납부 원천세·급여 누락 검토만 제공한다.
Stage E는 기존 월 급여 line에 이미 있는 급여 항목·4대보험·지방소득세까지 확장해 승인
Preview를 그리는 계약을 고정했고, Stage F에서 실제 화면에 연결한다.
...
6. **Annual Stage F — 다음**: 기존 급여·직원 DB를 승인 Preview에 연결하고 단위·정적·브라우저 QA와 문서 closeout을 완료한다. 신규 schema·mutation API·AI 없음.
