...
예외가 0건이면 긴 판단표를 보여주지 않고 완료 배너와 신고 준비 상태만 보여준다.
...
| 신고 패키지 미리보기 | 축소 | 대형 미리보기 카드는 제거하되 기존 package/rebuild gate와 사용자 액션은 compact 신고 준비 영역에서 유지한다. |
| 책임 경계 안내 | 축소 | 상시 대형 카드 대신 짧은 하단 문구와 필요한 위치의 도움말로 제공한다. |
| 판단 표 열·보조 문구 | 4열로 통합 | `거래/상대처`, `금액`, `AI 판단`, `홈택스·사용자 확정`만 둔다. AI 열은 판단 한 줄만, 홈택스 열은 할 일 한 줄과 `처리` 진입점 하나만 기본 표시한다. 출처·워크플로 상태·근거·증빙·재확인과 사용자 액션 묶음은 펼친 뒤 표시한다. |
...
그대로 runtime에 옮기되 VAT canonical 계산·mutation·undo·package gate를 삭제하지 않는다.
...
- [x] 예외 0건이면 긴 판단표 대신 완료 배너와 신고 준비 상태를 보여준다.
...
- [x] 삭제 후에도 사용자 확정·증빙 확인·되돌리기·gate·세액 계산은 유지된다.
