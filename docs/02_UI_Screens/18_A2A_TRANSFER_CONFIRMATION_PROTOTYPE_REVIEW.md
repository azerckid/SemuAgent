...
- 부가세 화면의 진입점은 Ready일 때만 노출하고 클릭 즉시 전송하지 않는다(마스터플랜 §8.17.4).
...
- 재확인 필요(fingerprint 불일치): warn 배너로 전환, `세무사무소로 전송` 비활성화,
  `부가세로 돌아가서 확인`만 활성
- Blocked(부가세 화면 진입점): Ready가 아니면 03_vat.html에 진입점 자체를 노출하지 않는다
  (별도 Preview 상태 아님, 기존 VAT package gate 재사용)
