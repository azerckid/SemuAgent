...
```text
[Flow]
판정: 원천세 1a 양식 없음 → 1b(직접입력 정리) 대상으로 결정. Field Mapping Part A는 1b 값 정리 소스로 활용됨(구현 완료)
Gate: 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) → 1a 파일 미해당, 1b로 제공
완료: 별지 제21호 A01 매핑 · NTS 공식 신고방법 감사 · 검증 패널 · A01 매핑을 1b `항목 = 값` 화면에 연결
...
공식 조사 결과 원천세는 공식 비암호화 업로드 양식이 없어 **1b(직접입력 정리) 대상으로 결정**됐다(값 정리 화면은 후속 구현).
이 문서의 Part A 서식 정합은 1b `항목 = 값` 화면의 소스이고, Part B·generator(1a)는 공식
양식이 확인될 때만 착수한다.
