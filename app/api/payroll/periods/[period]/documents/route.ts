...
import { client, payrollPeriodSummary } from '@/lib/db/schema'
...
export async function POST(
  ...
  try {
    ...
    if ((totals?.issueCount ?? 0) > 0) {
      return NextResponse.json({ error: `확인 필요 ${totals?.issueCount ?? 0}건 처리 후 생성할 수 있습니다.` }, { status: 409 })
    }
    ...
    await db
      ...
      .set({
        ...
        insuranceStatementStatus: 'generated',
        updatedAt: ts,
      })
    ...
    return NextResponse.json({ ok: true, period: period.data })
  } catch (err) {
    console.error('[POST /api/payroll/periods/[period]/documents]', err)
