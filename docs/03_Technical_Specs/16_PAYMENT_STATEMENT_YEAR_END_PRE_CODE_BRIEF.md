...
- **근로소득 지급명세서 신고 준비는 JC-030 Annual Stage A~F**다. Stage A는 공식
  ...
  이 문서의 Stage E 계약을 통과했다. 다음은 승인 화면을 실제 데이터에 연결하는
  **Stage F runtime·browser QA**다.
...
### 0.1 Annual Stage E Flow Status

```text
[Flow]
현재: Pre-Code Technical Brief
Gate: Stage D HTML Preview 오너 승인 + Stage E 데이터·상태·경계 계약 통과
완료: Annual Stage A 공식 경로 감사, B 법정 필드 매핑, C 정본/PII 계약, D Preview
다음: Annual Stage F — 기존 read model 확장, 승인 UI runtime 교체, 단위·브라우저 QA
...
- [ ] Stage F 연말정산 화면이 직원별 근무기간·급여/보험/기납부세액 준비값·홈택스 확인 항목·상태 3종을 표시하고, **정산액 계산은 하지 않음**을 명시한다.
- [ ] 중도입사·중도퇴사 직원은 급여 자료가 완결됐을 때 `특례 확인`으로 구분된다.
...
- [ ] Stage F runtime이 승인 Preview 구조와 동일한 직원별 급여 기초자료·상태 3종·행 상세를 실제 DB로 표시한다.
- [ ] Stage F 단위/정적/브라우저 QA에서 tenant·사업장·연도 격리, 합계 정합성, 상태 우선순위, 진행 중 연도, PII 비수집을 검증한다.
