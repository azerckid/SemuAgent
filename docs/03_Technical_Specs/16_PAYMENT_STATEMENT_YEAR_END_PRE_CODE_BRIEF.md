...
- **근로소득 지급명세서 신고 산출물은 JC-030**. Stage A는 공식 비암호화 업로드
  양식 미확인으로 Path 1b를 판정했고, 다음은 전체 필드·canonical 데이터 공백을
  확인하는 Stage B다. JC-024는 계속 데이터셋·검토 상태까지만 담당하며 현재
  연말정산 화면을 홈택스 직접입력 화면으로 간주하지 않는다.
...
- **Technical_Specs**: [근로소득 지급명세서 Stage A Audit](./55_JC030_ANNUAL_WAGE_STATEMENT_STAGE_A_AUDIT.md)
