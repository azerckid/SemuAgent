...
- Implementation Preconditions (착수 전 영향 감사 필수):
  ...
  - [x] Slice 3c-4a 구현 — `payroll_rule_profile_application`·`payroll_extraction_batch`·`payroll_extraction_row`·`payroll_excel_draft`에 nullable 범용 `source_batch_id` 추가(migration 0064), `source_batch.legacy_upload_session_id` 기준 backfill, 급여 추출/규칙 적용/엑셀 초안/first-run sample dual-write. `payroll_employee_line.source_batch_id`는 미변경. 머지 후 dev/prod DB 0064 적용·검증 필요.
- Acceptance Criteria:
...
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1~2c + Slice 3a~3b + Slice 3c-0 + **Slice 3c-1(2026-07-06, company-home read switch, PR #103)** + **Slice 3c-2(2026-07-06, validation/declaration FK, PR #104)** + **Slice 3c-3(2026-07-06, bookkeeping FK, migration 0063)** 구현 완료. **Slice 3c-4(2026-07-06, payroll lineage decision)** 완료. **Slice 3c-4a(2026-07-06, payroll extraction FK, migration 0064)** 구현 완료. 다음: PR 머지 후 dev/prod DB 0064 적용·검증, 이후 **Slice 3c-5** adaptive structuring allowlist/migration.
