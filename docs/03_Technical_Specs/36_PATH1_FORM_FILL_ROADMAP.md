...
다음 제품 작업: 근로소득 지급명세서 Stage B — 법정 필드 매핑·canonical 데이터 공백 계약(Preview/runtime 아님)
...
| 4 | **근로소득 지급명세서** | JC-024 연간 지급·기납부 원천세 준비만 존재 | Stage A 완료 — 직접작성/변환제출 확인, 공식 비암호화 업로드 양식 미확인; 전체 필드 정본 부족 | 업로드 파일·직접입력 화면 없음 | **Path 1b 대상 · Stage B 필드/데이터 공백 매핑 다음** |
...
현재 연말정산 read model은 연간 지급합계·기납부 원천세·급여 누락 검토만 제공한다.
근로소득공제, 과세표준, 소득/세액공제, 결정세액, 환급/추징 등 전체 지급명세서 필드의
canonical source가 없으므로 다음 작업은 화면이 아니라 **Stage B 필드 매핑·데이터 공백
계약**이다. 값이 없는 필드를 0이나 역산값으로 만들지 않는다.

1. **Annual Stage A — 완료**: 공식 경로 감사와 Path 1b 판정.
2. **Annual Stage B — 다음**: 법정/홈택스 필드 ↔ canonical source·사용자 입력·홈택스
   확인·v1 제외 매핑.
3. **Annual C 이후 — 대기**: Stage B가 정직한 v1 화면을 만들 수 있다고 판정한 뒤
   Preview·오너 승인·Pre-Code·runtime·QA.
