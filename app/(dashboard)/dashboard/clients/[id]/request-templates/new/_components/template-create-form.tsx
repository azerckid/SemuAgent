...
export function TemplateCreateForm({ clientId, clientName, checklistTemplates }: Props) {
  ...
  const handleExtractCriteria = async () => {
    ...
    if (!form.emailBody.trim()) {
      setCriteriaError('자료 요구사항 본문을 먼저 입력해 주세요.')
      return
    }
    setExtracting(true)
    try {
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-4 space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-1">
            <label className="block text-sm font-medium text-gray-700">본문 기준 정리</label>
            <button
              type="button"
          ...
          <textarea
            ...
            maxLength={10000}
            rows={6}
            placeholder="자료 요구사항 본문 작성 후 '본문 기준 정리'를 누르면 AI가 기준 초안을 정리합니다."
