...
- **실행 우선순위:** 신고 완료는 **Path 1(홈택스 업로드용 양식·파일 작성)** 이 최우선이다. Path 2(JC-034 ZIP) 구현은 Path 1a 베타 이후 ([Path 1 Roadmap](./36_PATH1_FORM_FILL_ROADMAP.md)).
...
2. Hero: 전체 준비율·확인 필요 건수·Path 1 양식 확인 준비 상태 (파생 지표, 신규 저장 없음)
3. **Path 1 신고 완료 경로** 섹션 (Path 2·3은 후속 경로로만 표시)
