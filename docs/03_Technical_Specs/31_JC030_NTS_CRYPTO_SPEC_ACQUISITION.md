...
> Last Updated: 2026-07-07 02:15 KST
...
| **D. 사용자 수동 암호화** | plain + 홈택스 전환 프로그램 안내 | v1 **현행**; 암호화 자동화 없이도 제출 가능(별도 도구) |
...
| 트랙 | 범위 | 상태 |
|---|---|---|
| **JC-030 v1** | plain A/B/C 생성·사전검증·다운로드·홈택스 안내 (Slices 1a–2a, 3) | **완결** — main, 실사용 가능 |
| **Slice 2b** | fcrypt 암호화·암호화 파일 다운로드·윈도우 microservice | **별도 트랙·보류** — DLL 실행·라운드트립 선행 |

v1 Acceptance Criteria 중 “업로드 가능한 전자신고 파일”은 **plain** 기준으로 충족 가능(홈택스가 암호화를 요구하면 사용자가 별도 전환 도구 사용 — 옵션 D). **암호화 자동화**는 2b 완료 시 추가.
