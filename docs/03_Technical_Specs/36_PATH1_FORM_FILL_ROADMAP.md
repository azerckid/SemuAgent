...
Path 2 (JC-044 JARYO 직접 A2A, JC-034 ZIP fallback)는 Path 1 전체 베타(1a+1b) 이후에만 재개한다. Path 3
...
보류: Path 2 직접 A2A·ZIP fallback, JC-023 자동제출
...
**Path 2(JC-044 주 경로, JC-034 fallback) 재개 게이트는 (b) Path 1 전체 베타다.**
두 전달 경로 모두 원천세·부가세 등 세목별 확정 summary와 Ready snapshot을 사용한다.
따라서 Path 2는 1a 파일 베타가 아니라 1b 값 정리까지 포함한 Path 1 전체 베타가 안정된
뒤에만 재개한다. JC-034 ZIP의 Path 1a 공식 비암호화 파일은 선택 첨부일 뿐이며,
JC-044 직접 A2A 역시 미완성 세목 summary를 전송하지 않는다.
...
- Path 2 JARYO direct A2A (JC-044) and manual ZIP fallback (JC-034)
...
- **Technical_Specs**: [JC-034 Pre-Code Brief](./35_JC034_GIWA_HANDOFF_PACKAGE_PRE_CODE_BRIEF.md) — Path 2 수동 ZIP fallback, 구현 대기
