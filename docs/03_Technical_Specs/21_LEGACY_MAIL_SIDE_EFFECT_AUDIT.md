...
```text
[Flow]
현재: JC-031 Slice 2b-3c 완료 — calendar/client outbound-email read cleanup
Gate: 통과
...
다음: transaction-purpose 내부 작업 재설계 또는 payroll event email-status read 분리
...
3. **Slice 2b-3 — Legacy mail UI/read cleanup**
   ...
   - **후속 분리:** payroll event email-status read는 payroll live 경로와 섞여 있어 별도 영향 감사 후 제거.
