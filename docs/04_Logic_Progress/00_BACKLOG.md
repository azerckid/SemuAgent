...
| JC-035 | todo | 부가세 AI 세무판단 보조 | `lib/vat`, `vat_deduction_review`, exact VAT fact, 기존 AI orchestration | **현재 제품 우선순위 · VAI-1 UI 확인 대기.** 공제/불공제/안분과 과세/영세율/면세 가능성을 공식 규칙·이전 확정 패턴·조건부 AI로 설명하고, 근거·필요 증빙을 보여준 뒤 사용자가 최종 확정한다. AI 자동확정·세무대리·직접입력 가이드·공식 규격 미확인 업로드 파일 생성은 제외. [Completion Contract](../03_Technical_Specs/44_VAT_AI_TAX_TREATMENT_COMPLETION_CONTRACT.md) · [Preview](../02_UI_Screens/previews/03_vat.html) |
...
- Status: `todo` — VAI-0 문서 계약 완료, VAI-1 UI-First Gate 확인 대기
...
- Prototype Review / 승인: [ ] — 기존 JC-011 화면 승인은 유지하되 JC-035 AI 판단 작업표는 별도 브라우저 확인 필요
- Product Boundary:
...
- Implementation Preconditions:
  ...
  - [ ] VAI-1 프로젝트 오너 브라우저 확인·승인
  - [ ] VAI-2 공식 규칙 매트릭스 작성 — 조문/시행령/국세청 안내 버전·적용일·필요 사실·필요 증빙
  - [ ] VAI-2 별도 Pre-Code Brief 작성·승인 — Zod/read model/API/mutation/audit/PII
- Acceptance Criteria:
...
- Document Sync Check: 2026-07-10 VAI-0 계약과 VAI-1 Preview 초안 작성. **구현 완료 아님.** UI 승인 뒤 VAI-2 규칙 매트릭스·Pre-Code Brief로 이동하며, JC-030 부가세 파일 Stage A는 병렬 외부 확인 대기로 유지한다.
