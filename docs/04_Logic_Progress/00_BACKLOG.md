...
- Status: `doing` — VAI-0~6 구현 완료, VAI-6 오너 브라우저 확인·PR 머지 뒤 `done` 전환
...
- Document Sync Check: 2026-07-11 VAI-6 구현 완료. validated 판단 행에서 사용자 미확정·보류·전문가 확인·필수 증빙·안분 미완료를 거래 행 단위 blocker로 파생하고, VAT 화면·rebuild API·package API가 같은 composite gate를 사용한다. API gate는 AI를 다시 호출하지 않으며, 사용자 확정 canonical VAT fact 또는 deduction decision만 통과한다. 신규 migration은 없다. 오너 브라우저 확인·PR 머지 뒤 JC-035를 `done`으로 전환하며, JC-030 부가세 양식 업로드 Stage A는 병렬 외부 확인 대기로 유지한다.
