...
> Created: 2026-07-14
> Status: owner approved · runtime implementation complete · browser verification pending
...
- 인증 후 기존 회사 활성화와 신규 온보딩 완료 경로를 `/dashboard`로 통일했다.
- 온보딩 API가 회사명과 랜덤 suffix로 30자 이하 Better Auth organization slug를 생성한다. 사용자는 기술 식별자를 입력하지 않는다.
- 설정 read model에서 `staff_mailbox` 조회와 업무메일 props를 제거했다. 기존 mailbox 데이터·API는 변경하지 않았다.
- 설정은 회사 정보·사용자 관리 2탭만 허용한다. 오래된 `mail`·`clients` query는 `resolveSettingsTab`에서 회사 정보로 안전하게 폴백한다.
- 사용자 표는 사업장 수·업무 메일함을 숨기고 이름·이메일·전화번호·권한·활성 상태만 표시한다.
- 세무 계산·자료수집·기장·부가세·급여·신고 준비·migration 변경은 없다.
