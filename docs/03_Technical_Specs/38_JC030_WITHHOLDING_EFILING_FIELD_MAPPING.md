...
```text
[Flow]
판정: 원천세 1a 양식 없음 → 1b(직접입력 정리) 대상으로 결정(값 정리 화면 미구현). Field Mapping Part A는 1b 값 정리 소스로 활용
Gate: 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) → 1a 파일 미해당, 1b 대상으로 결정(화면 미구현)
완료: 별지 제21호 A01 매핑 · NTS 공식 신고방법 감사 · 검증 패널
다음: A01 매핑을 1b `항목 = 값` 화면에 연결. 공식 양식이 확인되면 Part B(1a)로 승격
