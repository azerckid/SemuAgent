...
- 승인일: 2026-07-11
- 현재 구현: VAI-3a 공식 규칙·이전 확정 패턴, VAI-3b single-provider AI 보강, VAI-4a/4b 사용자 확정·되돌리기, VAI-5 조건부 multi-provider 합의에 이어 VAI-6a가 미확정·보류·전문가 확인·필수 증빙·안분 미완료를 신고 준비 검토 자료의 공통 잠금 사유로 연결한다. exact VAT fact가 없는 행은 억지로 추정하지 않고, 사용자 확정 전에는 rebuild/package를 차단한다.
- 확인 경로: `/dashboard/vat?period=2026-H1`의 영세율·불공제·안분·공제·면세 대표 행에서 액션 버튼, 판단 근거 대화상자, 최근 작업 되돌리기를 확인한다.
- 다음 작업: VAI-6b에서 영세율·면세 필수 증빙을 사용자가 확인 완료로 기록하고, 저장된 확인 결과가 재조회와 package gate에 반영되는 흐름을 구현한다. 그전에는 해당 매출 보유 사업자의 package 잠금을 유지한다.
