...
```text
[Flow]
현재: JC-030 원천세 Slice 1b-W0 — Field Mapping Part A 완료 · 공식 비암호화 업로드 양식 갭
Gate: blocked (JC-012/013 ↔ 별지 제21호 A01 매핑·UI·Slice 1a 완료, 홈택스 수용 업로드 양식 미확인)
완료: law.go.kr 별지 제21호(2024.3.22) · NTS 작성요령(2009) · 검증 패널
다음: 공식 비암호화 업로드 양식·수용 메뉴 확인 → W1 시트/셀/열 매핑 → Brief(39) 최종 승인
...
JC-030 Path 1 **2번 세목**은 **원천징수이행상황신고서** 비암호화 업로드 양식이다.
본 문서는 (A) **신고서 서식 필드**와 SemuAgent 데이터의 1차 매핑, (B) **공식 업로드 양식 위치** 갭을 기록한다.

현재 확인된 원천세 HWP/PDF는 작성 서식이며 홈택스가 직접 업로드받는 파일로
확인되지 않았다. v1 구현은 **서식 정합(집계)** 과 **비암호화 업로드 수용 확인**을
분리해 게이트한다.
