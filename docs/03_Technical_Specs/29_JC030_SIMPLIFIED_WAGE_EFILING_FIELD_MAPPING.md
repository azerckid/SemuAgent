...
다음: Pre-Code Brief 승인 → 구현 슬라이스(plain record builder → NTS-CRYPTO 래퍼)
...
**1차 매핑표**이다. 암호화(NTS-CRYPTO)·홈택스 적합성 검정은 [Pre-Code Brief](./30_JC030_EFILING_FILE_PRE_CODE_BRIEF.md)를 따른다.
...
| NTS-CRYPTO 암호화 | Brief §6 | 구현 슬라이스 2 |
