...
이 화면은 홈택스 업로드·신고 전 확인할 자료 상태를 보여준다. 공식 비암호화 업로드 양식이 확인되면 Path 1a(양식·파일)로, 없으면 Path 1b로 확정값과 신고 메뉴 경로·화면명·행/칸 위치를 함께 정리해 사용자가 홈택스에 직접 입력하도록 제공한다. 화면 캡처 기반 클릭별 튜토리얼과 자동 입력은 제공하지 않는다.
