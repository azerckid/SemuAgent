...
```text
[Flow]
현재: JC-031 — Impact Audit / Retirement Plan + Completion Contract
Gate: 진행 중
완료: Slice 1~1c 외부 포털 quarantine/dead code 제거, Slice 2a~2b-5 레거시 요청메일·outbound_email 런타임 생성/read side effect 제거
다음: Slice 2c transaction-purpose 내부 작업/FK 결정 → Slice 3 source batch replacement → Slice 4 schema retirement
필요 확인: transaction-purpose self-use 내부 작업 유지 여부, source_batch 모델 설계, schema retirement migration/retention 정책
