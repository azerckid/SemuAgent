...
- Implementation Preconditions (착수 전 영향 감사 필수):
  ...
  - [x] 업로드 포털 영향 — `/upload/[token]` 외부 포털 v1 제외 승인(2026-07-05). Slice 1 quarantine 구현 완료: `app/upload/[token]/layout.tsx` redirect 추가(JC-004와 동일 패턴), 컴포넌트는 미삭제. `/api/upload/*` namespace는 **공유 API**(`/api/upload`, `/api/upload/submit`, `/api/upload/files/[fileId]` 및 retry·password — direct-upload도 사용)와 **포털 전용 API**(`/api/upload/declarations`, `/api/upload/payroll-rule`, `/api/upload/purpose-request*` — 다음 slice 삭제 후보)로 나뉜다. 이번 slice는 공유 API를 포함해 어떤 API도 건드리지 않음.
...
- Document Sync Check: 2026-07-05 영향 감사 완료 + Slice 1(Route Quarantine) 구현 완료. `upload_session`은 현행 source lineage FK라 즉시 삭제 금지, 외부 포털/고객 요청메일 표면부터 단계적으로 격리. 구현: `app/upload/[token]/layout.tsx`(redirect, JC-004 패턴 재사용). `/api/upload/*` namespace는 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password, direct-upload 사용)와 포털 전용 API(`/api/upload/declarations`·`/api/upload/payroll-rule`·`/api/upload/purpose-request*`)로 나뉘며(리뷰 반영, 2026-07-05), 이번 slice는 어떤 API도 삭제하지 않음. 컴포넌트(`upload-portal.tsx` 등)는 다음 slice에서 정리. 다음: 포털 전용 API·컴포넌트 정리 slice 또는 Slice 2(메일 은퇴).
