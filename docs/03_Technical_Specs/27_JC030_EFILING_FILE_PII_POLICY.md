...
```text
[Flow]
현재: JC-030 PII 정책 유지 — 식별정보 영구 저장 금지, 필요 시 일회성 처리
Gate: 세목별 공식 비암호화 업로드 양식이 식별정보를 요구할 때 동일 정책 적용
완료: 간이지급 정책 + 근로소득 지급명세서 Stage C 홈택스 직접처리 경계
다음: 새 세목 W0 또는 승인된 Preview가 PII를 요구할 때 별도 재검토
...
## 7. Annual Wage Statement Stage C Extension

[Annual Wage Stage C Contract](./57_JC030_ANNUAL_WAGE_STATEMENT_CANONICAL_SOURCE_CONTRACT.md)은
홈택스 편리한 연말정산이 근로자 기초자료와 공제신고서를 결합해 최종 지급명세서를
생성·수정하는 흐름임을 확인했다.

근로소득 지급명세서 Path 1b v1은 간이지급 파일 후보와 달리 주민등록번호를 SemuAgent
서버에 일회성으로도 받지 않는다.

- 주민/외국인등록번호는 홈택스에서 직접 입력한다.
- 연말정산간소화 PDF·공제신고서·가족/계좌/공제증빙을 SemuAgent에 업로드하지 않는다.
- 홈택스가 생성한 개인별 지급명세서 PDF/HWP를 파싱·저장하지 않는다.
- SemuAgent는 내부 직원 식별자와 비민감 급여 기초자료만 화면에서 집계한다.
- 최종 공제·결정세액·차감징수세액은 홈택스 생성 결과를 정본으로 둔다.

향후 공식 machine-readable 결과 export를 도입하려면 별도 PII·무결성·삭제·접근통제
brief와 법무 승인이 필요하다. 현재 정책으로 자동 확장하지 않는다.
