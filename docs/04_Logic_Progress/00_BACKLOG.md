...
- Implementation Preconditions (조사·설계 과제):
  ...
  - [ ] **근로소득 지급명세서 Stage D 오너 확인** — 직원 표의 정보 밀도·상태·행 상세 구조를 확인한다. 승인 전 runtime·schema·API 금지
  - [ ] **근로소득 지급명세서 Path 1b Runtime~Closeout** — 승인 Preview 기준 Pre-Code·read model·필요 시 신고 profile·browser/QA·문서 closeout
...
- Document Sync Check Update (2026-07-14): **근로소득 지급명세서 Stage C canonical source 계약을 완료했다.** 국세청 편리한 연말정산은 회사 기초자료와 근로자 공제신고서를 결합해 지급명세서를 생성하고, 회사가 확인·수정 후 별도로 제출한다. SemuAgent는 확정 급여 기초자료 준비만 담당하고 최종 공제·결정세액·차감징수세액은 홈택스 생성 결과를 정본으로 둔다. 청구 profile은 신고 profile로 재사용하지 않으며 주민번호·가족·공제증빙·최종 PDF/HWP를 수집·저장하지 않는다. 다음은 Stage D HTML Preview이며 오너 승인 전 runtime·schema·API를 만들지 않는다.
- Document Sync Check Update (2026-07-14, Stage D Preview): **근로소득 지급명세서 Stage D HTML Preview를 제작했다.** 기존 `연말정산` 메뉴 아래 한 화면에서 홈택스 편리한 연말정산의 기초자료→공제신고서→지급명세서 생성→확인·수정→제출 흐름과 직원별 급여 준비값을 보여준다. 상태는 `급여 준비 완료/급여 보완/특례 확인` 3개만 사용하고, 최종세액·환급/추징·주민번호·공제증빙 입력·중복 설명 카드는 포함하지 않았다. 현재는 오너 확인 대기이며 승인 전 runtime·schema·API를 만들지 않는다.
