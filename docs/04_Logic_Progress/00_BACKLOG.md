...
- Status: `doing` — VAI-0~6a 구현 완료, VAI-6b 영세율·면세 증빙 확인 입력 완료 뒤 `done` 전환
...
- Acceptance Criteria:
  ...
  - [ ] 사용자가 영세율·면세 필수 증빙을 확인 완료로 기록하고 확인자·시각을 감사할 수 있다.
...
- Document Sync Check: 2026-07-11 VAI-6a 구현 완료. validated 판단 행에서 사용자 미확정·보류·전문가 확인·필수 증빙·안분 미완료를 거래 행 단위 blocker로 파생하고, VAT 화면·rebuild API·package API가 같은 composite gate를 사용한다. API gate는 AI를 다시 호출하지 않으며, 사용자 확정 canonical VAT fact 또는 deduction decision만 통과한다. 신규 migration은 없다. 현재 영세율·면세 필수 증빙을 `present`로 기록하는 사용자 입력 경로가 없어 해당 매출 보유 사업자는 package가 계속 차단된다. VAI-6b에서 증빙 확인 입력·감사 기록·gate 재계산을 구현한 뒤에만 JC-035를 `done`으로 전환한다. JC-030 부가세 양식 업로드 Stage A는 병렬 외부 확인 대기로 유지한다.
