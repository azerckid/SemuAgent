...
9. **세무 일정(Tax Schedule)** — UI Preview 작성(2026-07-04, JC-029·전체 세무 의무 가시화 허브)
...
## 4g. 세무 일정 화면 흐름 및 데이터 (JC-029)

진입: 사이드바 "세무 일정". 회사가 챙겨야 할 세무 전체를 한 화면에서 개요로 본다(read-only 허브).
연계: 각 항목에서 해당 워크스페이스(부가세·급여·신고지원 등)로 이동. 리마인드(JC-016) 기한과 연동.

구성(3단):
- 상단 = 이번 달·분기 해야 할 일(기한·D-day·상태)
- 중간 = 세무 의무 전체 커버리지(세목·주기·다음 마감일·상태[구현됨/준비중/예정]·연결 워크스페이스)
- 하단 = 준비 중 / 예정 / 법무 검토(로드맵 JC-024~028, JC-023)

사업자 유형(개인/법인/면세)에 따라 해당 없는 세목(예: 법인의 종합소득세·사업장현황신고)은 흐림/숨김. 신고·제출 자체는 각 워크스페이스·홈택스에서 수행하며 이 화면은 개요·안내다(자동제출은 JC-023 원칙).
...
- Preview (세무 일정): [08_tax_schedule.html](./previews/08_tax_schedule.html)
