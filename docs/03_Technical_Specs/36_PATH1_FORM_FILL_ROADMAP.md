...
현재: 부가세 Stage A — 공식 비암호화 업로드 양식·홈택스 직접 수용 경로 감사
...
| 2 | **부가가치세** | 자료대조 Phase 2·VAT deduction·provenance 완료 | 일부 부속명세 파일변환 확인; 전체 신고 비암호화 공식 양식·규격 미확인 | 업로드 파일 생성 없음 | **현재 Stage A, partial / blocked** |
...
1. **VAT A** — [공식 비암호화 업로드 양식 감사](./43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md). 일부 부속명세 파일변환은 확인했지만 전체 신고 양식·규격·직접 수용 경로는 `partial / blocked`다.
...
- [x] [VAT Stage A Audit](./43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md) — 일부 부속명세 확인, 전체 신고 양식 `partial / blocked`
