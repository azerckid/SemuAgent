...
Path 2는 Path 1 베타 테스트 이후 검토한다. Path 3 인증·암호화 파일은 현재 제품 범위에서 제외한다.

| Path | 이름 | SemuAgent 역할 | 제출 주체 | 백로그 | 시점 |
|---|---|---|---|---|---|
| **1** | 공식 비암호화 양식 파일 + 홈택스 업로드 안내 | 홈택스가 직접 수용하는 공식 양식에 값 입력·검증 + 홈택스 업로드 단계 안내 | 사업자 | JC-013, JC-030 Path 1 | **현재·베타 범위** |
...
홈택스·국세청이 제공하고 홈택스가 직접 수용하는 **공식 비암호화 업로드 양식**이
있는 세목에 한해 SemuAgent가 확정 데이터로 **파일을 만들어 준다**. 사용자는
홈택스에서 **직접 업로드·제출**한다.

- JC-013: 단계별 준비값 확인·접수증 보관
- JC-030 Path 1: 공식 비암호화 업로드 양식 생성·사전검증·홈택스 업로드 안내
- Path 1은 **고급 옵션이 아니다**. 수임 사무소가 없거나 직접 신고를 선택하는 정식 경로다.
- 공식 비암호화 업로드 양식이나 직접 수용 경로가 확인되지 않은 세목은 `blocked`로 두고 파일을 생성하지 않는다.
...
|---|---|---|
| 1 | **최우선** — 공식 비암호화 업로드 양식 입수·기입·신고 보조 | 세목 확대: 원천세 W0 확인 → 호환 세목 순차 확대 ([Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md)) |
