...
- Status: `doing` — VAI-0~6a 머지 완료, VAI-6b 코드 구현·dev migration 완료·prod/브라우저/PR 대기
...
- Document Sync Check: 2026-07-11 VAI-6a는 PR #199로 머지 완료. VAI-6b는 거래 행·증빙 코드별 additive 감사 테이블(migration `0070`), 확인 완료·확인 취소 mutation, 활성 확인 기록의 `present` 합성, fingerprint·gate 재계산, VAT 판단 표 인라인 액션을 구현했다. 확인 취소는 감사 row를 삭제하지 않고 `revoked`로 보존한다. migration `0070` dev 적용과 컬럼·인덱스 확인은 완료했으며, prod 적용·브라우저 E2E·PR 머지 전에는 JC-035를 `done`으로 전환하지 않는다. JC-030 부가세 양식 업로드 Stage A는 병렬 외부 확인 대기로 유지한다.
