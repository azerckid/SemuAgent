...
다음: 공식 비암호화 직접 업로드 수용 여부 재검증 — 암호화 경로는 현재 제품 범위 밖
...
**1차 매핑표**이다. 현재 생성 파일은 비암호화 후보이며, 공식 홈택스 직접 수용
여부를 검증하기 전에는 Path 1 완료로 판정하지 않는다.
...
| fcrypt 암호화 | [Archived NTS Crypto Research](./31_JC030_NTS_CRYPTO_SPEC_ACQUISITION.md) | 현재 제품 범위 밖 |
