...
> Status: draft · Brief 승인 후 구현과 함께 Result 갱신
...
| UX | Pending | 비차단 업로드, disabled Instant/Mic/Voice, 거절 문구 |
...
| # | Given | When | Then | Result |
|:---|:---|:---|:---|:---:|
| U-01 | 허용 PDF ≤50MB | 세비서 첨부 | `staff_direct` 세션·`upload_file` 생성, 자료수집 표에도 동일 행 | Pending |
| U-02 | 허용 XLSX | 첨부 후 submit | status uploaded→analyzing(또는 파이프라인 동등 전이) | Pending |
| U-03 | 미지원 형식 | 첨부 시도 | 거부 메시지, DB 행 없음 | Pending |
| U-04 | >50MB | 첨부 시도 | 거부, DB 행 없음 | Pending |
| U-05 | 분석 실패 파일 | 재시도 CTA | 기존 retry API 호출, 상태 갱신 | Pending |
| U-06 | 암호 Excel | 비밀번호 제출 | 기존 password API, 비밀번호가 thread에 평문 잔존하지 않음 | Pending |
...
| C-02 | 활성 composer | “오늘 날씨 어때?” | 거절만, 업무 답변 없음, LLM 장문 생성 없음(또는 거절 템플릿) | Pending |
...
| N-03 | 업로드 완료 | 자동 장문 LLM 요약 | Brief 기본안(끔)이면 호출 없음 | Pending |
...
| P-02 | 450px 폭 | composer·참고 일정 | 가로 스크롤 없이 사용 가능 | Pending |
| P-03 | 키보드 | Tab/Enter | 입력·전송 가능, disabled 컨트롤 포커스 시 준비 중 안내 | Pending |

## 9. Exit Criteria For CUI-3

- [ ] T-01~T-05 회귀 PASS
- [ ] U-01~U-08 및 자료수집 S-60~S-64 회귀 PASS
- [ ] C-01~C-08 PASS
- [ ] R-01~R-03 PASS
- [ ] I-01~I-06 PASS
- [ ] N-01~N-04 PASS
