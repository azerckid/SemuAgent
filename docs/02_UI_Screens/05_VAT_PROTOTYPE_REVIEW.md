...
- 승인일: 2026-07-11
- 현재 구현: VAI-3a 공식 규칙·이전 확정 패턴, VAI-3b single-provider AI 보강, VAI-4a/4b 사용자 확정·되돌리기, VAI-5 조건부 multi-provider 합의에 이어 VAI-6가 미확정·보류·전문가 확인·필수 증빙·안분 미완료를 신고 준비 검토 자료의 공통 잠금 사유로 연결한다. exact VAT fact가 없는 행은 억지로 추정하지 않고, 사용자 확정 전에는 rebuild/package를 차단한다.
- 확인 경로: `/dashboard/vat?period=2026-H1`의 영세율·불공제·안분·공제·면세 대표 행에서 액션 버튼, 판단 근거 대화상자, 최근 작업 되돌리기를 확인한다.
- 다음 작업: VAI-6 잠금 사유를 프로젝트 오너가 브라우저에서 확인하고 PR 머지 뒤 JC-035를 최종 완료 처리한다.
