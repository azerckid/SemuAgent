...
> Status: Draft — Brief 승인 후 runtime 검증에 사용
...
| T-06 | tenant 격리 | tenant B sessionId 주입 | sessionId strip. 타 tenant 행 0 | Pending |
| T-07 | 사업장 격리 | 다른 client sessionId | strip. 0건 | Pending |
| T-08 | period 불일치 | 세션 period와 다른 period query | strip / fail-closed | Pending |
| T-09 | 무효 sessionId | 없는 UUID | sessionId strip. 기존 기간 ledger 또는 빈 안전 상태(Brief §4.3) | Pending |
...
- S2 Preview 미승인 시 runtime 착수하지 않는다(Brief §9).
- 확인 환경: 로컬 우선. staging 로그인이 복구되기 전에는 staging을 필수 경로로 적지 않는다.
