...
1b 완료: 근로소득 간이지급 — 직접작성 경로·사업자/기간·소득자별 월 지급액·합계; 파일 후보 UI 제거
1b 완료: 원천세 — 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) → 직접입력 안내 화면 구현
1b 완료: 부가세 — 전체 신고 비암호화 양식 미확인 → Field Mapping·Preview·runtime·browser 검증 완료; 1a는 [Stage A 확인](./43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md) 시
1a 후보·외부 대기: 지방소득세 특별징수 — 위택스 공식 Excel 원본은 로그인 뒤 입수 필요
1b 판정·Stage C 완료: 근로소득 지급명세서 — 급여 기초자료는 SemuAgent, 최종 지급명세서는 홈택스 정본
다음: 근로소득 지급명세서 Stage D — 간결한 기초자료 준비 HTML Preview·오너 승인
조건부 후순위: 사업장현황신고 — 면세 개인사업자 대상성 gate와 fixture 우선
보류: JC-044 Path 2 직접 A2A, JC-034 ZIP fallback 구현
...
Path 2의 주 경로인 JARYO 직접 A2A는 **JC-044**, 수동 ZIP fallback은 **JC-034**가 담당하며
둘 다 JC-030 Validation 출력을 소비한다.
...
| 후보 | 장점 | 차단/리스크 | 판정 |
|---|---|---|---|
| 근로소득 간이지급명세서 | JC-024 데이터가 live, 공식 직접작성 제출 경로 확인 | 공식 비암호화 직접 업로드 양식 미확인 | **1b 직접입력 정리 구현 완료** |
| 원천세 신고서 | 급여/원천 데이터 live, 지방소득세 정합성까지 완료, 월 신고 | 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) | **1b 직접입력 정리 구현 완료** |
| 부가가치세 | SemuAgent의 핵심 세목, 직원 PII 의존 낮음, Phase 2 gate/provenance 완료 | 전체 신고 비암호화 양식 미확인(회계프로그램 변환·일부 첨부서류만) | **1b 직접입력 정리 구현 완료; 1a는 Stage A 확인 시** |
| 지방소득세 특별징수 | JC-027 실제 저장 세액 live, 위택스 공식 Excel 파일신고 확인 | 로그인 원본 `B070101-02.xlsx`의 시트·열·유효성 미확인 | **Path 1a 후보 · 원본 입수 대기** |
| 근로소득 지급명세서 | JC-024 월 확정 급여·직원 명부 + Stage B 현행 제24호서식 매핑 | Stage C 홈택스 정본, Stage D 승인 Preview, Stage E 계약, Stage F runtime·QA 완료 | **Path 1b 완료** |
| 사업장현황신고 | JC-028 read-only 준비 화면 | 면세 개인사업자만 대상; 대상성 gate·공식 양식 감사 미착수 | **조건부 후순위** |
| 법인세/종합소득세 | 커버리지 확장 가치 큼 | JC-025/026 미구현, 법무/세무조정 리스크 큼 | 후순위 |

## 4. v1 Direction

JC-030 **1차 구현 세목:** 근로소득 **간이지급명세서 Path 1b**. 공개 공식 안내의
직접작성 경로와 확정 급여 값을 한 화면에 정리한다. 과거 고정길이 후보 파일은 활성
제품 경로가 아니다.

**다음 확대:** 원천세·부가세는 1b(직접입력 정리) 구현을 완료했다. 지방소득세
특별징수는 위택스 공식 Excel 파일신고가 확인되어 Path 1a 후보지만, 로그인 원본을
입수할 때까지 외부 대기한다. [Annual Wage Statement Stage B Mapping](./56_JC030_ANNUAL_WAGE_STATEMENT_FIELD_MAPPING.md)은
현행 제24호서식의 필드·데이터 공백 매핑을 완료했다. [Stage C](./57_JC030_ANNUAL_WAGE_STATEMENT_CANONICAL_SOURCE_CONTRACT.md)는
공제신고서와 최종 지급명세서가 홈택스 정본이며 SemuAgent는 확정 급여 기초자료만
준비한다고 결정했다. Stage D Preview 오너 승인, Stage E Pre-Code와 기존 급여·직원
데이터를 연결한 Stage F read-only runtime·QA를 완료했다.
사업장현황신고는 면세 개인사업자 조건부 후순위다. 순서·게이트는
[Path 1 Form Fill Roadmap](./36_PATH1_FORM_FILL_ROADMAP.md)을 따른다.

### Validation (Path 1 & 2 공통)

1. JC-024 live data → 직접작성 값 준비 검증 via `lib/efiling-simplified-wage`.
2. Output feeds Path 1b read-only screen. Path 2 JC-044 direct A2A와 JC-034 ZIP fallback은
   Path 1 세목 확대 안정 후 재사용.

### Path 1 — 양식이 있으면 1a, 없으면 직접작성 1b

1. 공식 비암호화 업로드 양식과 직접 수용 메뉴가 확인된 세목만 1a 파일을 생성한다.
2. 확인되지 않은 세목은 확정값과 홈택스 메뉴·화면 위치를 제공하는 1b로 구현한다.
3. 근로소득 간이지급명세서와 원천세는 현재 1b 구현 완료다.
...
결정이었지만, 과거 근로소득 지급명세서 파일 후보에는 소득자 식별정보가 필요했다.

**결정(2026-07-06, JC-030 Slice 0b):** [JC-030 E-Filing File PII Policy](./27_JC030_EFILING_FILE_PII_POLICY.md) — **옵션 1 서버 미저장 일회성 입력** 채택.

- 현재 Path 1b 화면에서는 식별정보를 입력받지 않고, 홈택스 직접작성 화면에서 사용자가 입력한다.
- 향후 공식 Path 1a 파일이 확인되는 경우에만 파일 생성 직전 일회성 입력 정책을 재사용한다.
- `employee_profile`에 주민등록번호 컬럼을 **추가하지 않는다.**
- 과거 후보 생성기는 사용자 화면에서 제거하며, 생성 파일을 활성 제품 산출물로 제공하지 않는다.
...
- **Technical_Specs**: [JC-034 GIWA Handoff Scope Gate](./34_JC034_GIWA_HANDOFF_PACKAGE_SCOPE_GATE.md) — 수동 ZIP fallback, 구현 보류
