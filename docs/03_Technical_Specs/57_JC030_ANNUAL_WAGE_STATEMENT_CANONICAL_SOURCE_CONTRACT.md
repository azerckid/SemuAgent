...
UI-First Gate: Stage D HTML Preview 오너 승인 완료(2026-07-14), runtime은 Stage E Pre-Code 전 금지
다음: Annual Stage E — Screen Flow·UI Design·Pre-Code Brief 동기화
...
이 값들은 화면을 그릴 때 기존 정본에서 다시 계산한다. Stage E Pre-Code 완료 전에는 새 저장
...
간결한 홈택스 생성 준비 화면에만 적용된다. Stage E에서 Screen Flow·UI Design·Pre-Code
Brief를 동기화하기 전에는 runtime·schema·migration·API를 구현하지 않는다.
