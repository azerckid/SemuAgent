...
수정 대상이 0건이면 작업대 섹션 전체를 렌더하지 않고 Hero 한 줄만 유지한다.
수정 대상이 있으면 **신고 전 수정 필요 N건**을 우선 표시하고 `expected_no_change`는 기본 작업대에 펼치지 않는다.
...
1. 예상 납부·환급 세액
2. 지금 처리할 거래
3. 수정 대상이 없으면 추가 작업대를 표시하지 않는 완료 상태
...
| 매입세액 공제 검토 표 | 통합 | AI 판단 행과 기존 공제 검토 행을 `신고 전 수정 필요` 작업대 한 곳에서 처리한다. 기존 mutation은 유지한다. |
...
| 수정 작업대 | 유지·축소 | 홈택스 수정 행동·unresolved handoff·미완료 공제 검토만 기본 노출한다. `expected_no_change`는 숨기고 0건이면 섹션을 렌더하지 않는다. |
...
2026-07-13 VUI-1d 오너 결정으로 `추가 공제 가능성`을 절세 핵심 영역으로 유지하고 일반 예외 큐를
`신고 전 수정 필요`로 좁혔다. AI 출처·미확정만으로는 표시하지 않으며 실제 홈택스 수정 행동,
unresolved handoff, pending 공제 검토만 남긴다. 모바일은 고정 Sidebar 대신 동일 메뉴를 재사용하는
Sheet를 사용한다.
...
- [x] 담당자 이관은 JC-039 Human Handoff Gate를 통과한 행에만 표시한다.
...
- [x] desktop/mobile에서 표·버튼·팝오버 겹침과 수평 페이지 overflow가 없다.
- [x] Preview·Prototype Review·UI Design·Backlog·QA가 runtime과 일치한다.

VUI-1d는 데스크톱 1400px과 모바일 450px에서 document scrollWidth와 clientWidth가 일치함을
확인했다. `md` 미만에서는 고정 Sidebar를 숨기고 공용 메뉴를 재사용하는 320px Sheet를 열고 닫는다.
VAT 표는 내부 `overflow-x-auto`에 격리되며 샘플 배너·본문·Hero는 모바일 폭에서 겹치지 않는다.
