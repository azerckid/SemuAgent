...
6. NTS-CRYPTO 암호화 래퍼(8~15자리 사용자 비밀번호) — **슬라이스 2**; 슬라이스 1은 plain + 검증만
...
| Crypto | `lib/efiling-simplified-wage/encrypt.ts` (슬라이스 2, NTS-CRYPTO 스펙 입수 후) |
...
- Plain 레코드 파일 → NTS-CRYPTO → 사용자 비밀번호 8~15자리
...
| 슬라이스 | 내용 | 착수 조건 |
|:---|:---|:---|
| **1a** | `build-records` 순수 함수 + V-01~V-11 테스트 | Brief 승인 |
