...
1b 전환: 부가세 — 전체 신고 비암호화 양식 미확인 → 직접입력 정리로 제공(1a 양식은 Stage A 확인 시 추가)
다음 제품 작업: 원천세·부가세 1b 직접입력 정리 화면 정합 → 지방소득세 특별징수 Stage A
...
**1b(직접입력 정리)로 제공**한다. 세목을 `blocked`로 두지 않는다.
...
1. 근로소득 간이지급명세서가 §2.1(1a 파일)을 통과한다. 나머지 세목은 1a 양식이
   확인되면 1a로, 아니면 1b(직접입력 정리)로 제공한다.
...
| 1 | **원천세 신고서** | JC-012·JC-013 | W0 공식 조사 완료; 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) | 1a 파일 없음 · 1b 값 정리 대상 | **1b 직접입력 정리 지원 (1a 양식 미확인)** |
| 2 | **부가가치세** | 자료대조 Phase 2·VAT deduction·provenance 완료 | 현재 홈택스 `파일 변환신고(회계프로그램)`와 2020년 일부 첨부서류 Excel 도구 확인; 전체 신고 비암호화 양식 미확인 | 1a 파일 없음 · 1b 값 정리 대상 | **1b 직접입력 정리 제공; 1a 양식은 Stage A 확인 시** |
...
| **1b** | 확정 A01 집계를 `항목 = 값`으로 직접입력 정리 화면 제공 | 기존 검증 패널 + 값 정리 화면; 파일 generator 없음 |

원천세는 [공식 근거 감사](./37_JC030_WITHHOLDING_EFILING_LAYOUT_ACQUISITION.md)상
공식 비암호화 업로드 양식이 확인되지 않아 **1b(직접입력 정리)로 제공**한다. 최신 공식
양식과 직접 수용 메뉴가 새로 확인되면 그때 1a(파일 W1~W5)로 승격한다.
...
### 4.3 VAT — 1b 직접입력 정리(기본) + 1a 양식(Stage A 확인 시)

부가세는 공식 비암호화 전체 신고 양식이 아직 확인되지 않아 **기본 1b(직접입력
정리)로 제공**한다. 1a 파일 트랙은 Stage A 외부 확인이 통과할 때만 추가한다.

1. **1b (현재)** — 자료대조 Phase 2·VAT provenance·JC-035 확정 판단으로 만든 확정값을 `항목 = 값`으로 화면에 정리한다. 파일 generator 없음.
...
3. **VAT B~G (1a)** — Stage A에서 공식 양식이 확인되면 그때만 Phase 2 gate/provenance를 generator가 소비한다. 확인되지 않으면 1b 직접입력 정리를 유지한다(`blocked` 아님).
...
- [x] [VAT Stage A Audit](./43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md) — 전체 신고 비암호화 양식 미확인 → 부가세 1b 직접입력 정리 제공, 1a는 Stage A 확인 시
