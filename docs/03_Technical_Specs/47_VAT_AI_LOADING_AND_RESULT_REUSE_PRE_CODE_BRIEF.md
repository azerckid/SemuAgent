...
[JC-039 VAI-8a](./50_VAT_AI_EVIDENCE_BACKED_DECISIVE_JUDGMENT_BRIEF.md)는
`provisionalJudgment`와 `judgmentWorkflowStatus`를 payload v2로 분리했으며, v1 저장 결과는
후속 migration 없이 version 불일치로 재사용하지 않는다. `evidenceTrace`는 VAI-8b에서 같은
versioned payload 경계로 확장한다.
