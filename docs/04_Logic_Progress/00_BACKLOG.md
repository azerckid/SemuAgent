...
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1(Route Quarantine) + Slice 1b(죽은 컴포넌트/lib 삭제 + 포털 전용 API 4개 410 차단) + Slice 1c(`session.ts` 포털 조회 dead code 제거) + Slice 2a(레거시 고객 요청메일 쓰기 API 12개 410 차단) + Slice 2b 영향 감사 + Slice 2b-1(`missing_request` runtime side effect 제거 및 `missing-requests` GET 410) + Slice 2b-2(transaction-purpose send 410, `sent_email_id` FK 유지) + Slice 2b-3a(hidden reviews 보충요청 메일 read panel 제거) + Slice 2b-3b(hidden `dashboard/emails` mail-console read surface 제거) + Slice 2b-3c(calendar/client outbound_email read 제거) + Slice 2b-3d(dead payroll event loader outbound_email read 제거) + Slice 2b-3e(redirect-blocked `clients/[id]/events` detail/new read UI 제거) + Slice 2b-4(stale `missing_request` draft cleanup side-effect 제거) + Slice 2b-5(transaction-purpose send dead-code 제거) + Slice 2c(`sent_email_id` FK 제거·purpose draft API 410·dead service/UI 삭제·classification answer/apply 유지) 완료. 2026-07-05 Slice 3 Pre-Code Brief 작성 및 Slice 3a 구현: `source_batch`를 새 내부 source lineage 모델로 도입하고, `upload_file.source_batch_id` nullable FK·backfill migration 0061·direct-upload dual-write를 적용했다(dev DB 0061 검증 완료). `upload_session`은 Slice 3에서 삭제하지 않고 compatibility table로 유지하며, 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password), direct-upload, work-email, internal reminder는 계속 미변경. `lib/session.ts`는 토큰 검증(`verifyToken`)만 남기고, 포털 렌더링용 `getPortalData`와 하위 헬퍼는 제거. `dashboard/emails`는 redirect leaf로 축소했고, 설정 화면의 업무메일 주소 관리 컴포넌트와 work-email API는 보존. calendar/client 목록, 죽은 payroll event loader, redirect-blocked client event detail/new UI, upload revision/criterion review side-effect, transaction-purpose send service 및 Slice 2c에서 제거한 purpose draft UI/API/service는 더 이상 `outbound_email`으로 메일 실패·초안 대기·발송 기록·draft reject 상태를 만들지 않는다. 다음: Slice 3b read model switch → Slice 3c downstream FK migration → Slice 4 schema retirement.
