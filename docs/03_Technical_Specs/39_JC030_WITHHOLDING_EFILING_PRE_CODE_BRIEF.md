...
제외: 바이너리 레코드 추정, fcrypt·암호화 파일, 화면 캡처 기반 클릭별 튜토리얼, 자동 입력
```
...
- 화면 캡처 기반 클릭별 튜토리얼·자동 입력·자동제출 (신고 메뉴 경로·화면명·A01 행/칸 위치 매핑은 1b 필수)
...
| **1b branch** | 공식 양식 없음 | 1a generator 미구현. 홈택스 메뉴 경로·기본정보·A01 ④⑤⑥ 위치와 확정값을 함께 표시하는 직접입력 안내 화면은 **구현 완료** |

Path 1a는 W0부터 W5까지 완료되어야 끝난다. W0·W1 전에는 W2 generator
코드를 작성하지 않는다. 공식 양식이 없는 1b branch는 아래 별도 완료선을 따른다.

### 7.1 Path 1a Completion Line
...
- [ ] 사용자가 직접 업로드·제출하며 자동입력·자동제출 문구가 없다.
- [ ] QA·Backlog·Completion Contract·Audit가 main 상태와 일치한다.

### 7.2 Path 1b Completion Line

- [x] 홈택스 메뉴 경로·기본정보·A01 ④⑤⑥ 행/칸 위치와 live 값이 함께 표시된다.
- [x] 지급연월은 실제 급여 지급일에서 파생하며, 지급일이 없으면 같은 달로 추정하지 않고 확인 필요로 표시한다.
- [x] 지방소득세는 홈택스 합계에서 분리해 위택스 별도 신고로 표시한다.
- [x] 자동입력·자동제출·자동납부를 제공하지 않고 사용자 직접 수행 경계를 표시한다.
- [x] 제출 후 원천세 접수증 PDF 보관 경로를 제공한다.
- [x] 다른 세목 패키지·중복 준비 단계·혼합 사후 체크리스트를 원천세 화면에서 제거한다.
