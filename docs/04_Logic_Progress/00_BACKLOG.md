...
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1(Route Quarantine) + Slice 1b(죽은 컴포넌트/lib 삭제 + 포털 전용 API 4개 410 차단) + Slice 1c(`session.ts` 포털 조회 dead code 제거) + Slice 2a(레거시 고객 요청메일 쓰기 API 12개 410 차단) + Slice 2b 영향 감사 + Slice 2b-1(`missing_request` runtime side effect 제거 및 `missing-requests` GET 410) + Slice 2b-2(transaction-purpose send 410, `sent_email_id` FK 유지) + Slice 2b-3a(hidden reviews 보충요청 메일 read panel 제거) + Slice 2b-3b(hidden `dashboard/emails` mail-console read surface 제거) + Slice 2b-3c(calendar/client outbound_email read 제거) + Slice 2b-3d(dead payroll event loader outbound_email read 제거) 완료. `upload_session`은 현행 source lineage FK라 즉시 삭제 금지 원칙 유지. 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password), direct-upload, work-email, internal reminder는 계속 미변경. `lib/session.ts`는 토큰 검증(`verifyToken`)만 남기고, 포털 렌더링용 `getPortalData`와 하위 헬퍼는 제거. `dashboard/emails`는 redirect leaf로 축소했고, 설정 화면의 업무메일 주소 관리 컴포넌트와 work-email API는 보존. calendar/client 목록과 죽은 payroll event loader는 더 이상 `outbound_email`으로 메일 실패·초안 대기 상태를 만들지 않는다. 남은 live event detail 발송 기록 read는 요청 메일 발송 UX와 함께 별도 정책 결정 후 정리. 다음: transaction-purpose 내부 작업 재설계 또는 live event detail 메일 read 정리, 이후 source lineage 이관 및 schema retirement gate.
