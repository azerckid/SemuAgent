...
아래 시나리오는 [VAT AI Tax Treatment Completion Contract](../03_Technical_Specs/44_VAT_AI_TAX_TREATMENT_COMPLETION_CONTRACT.md)의
VAI-2 Pre-Code Brief 승인 뒤 구현한다. 현재 결과는 모두 `Pending`이다.
...
- **JC-035 자동화 계획**: VAI-2에서 규칙 fixture·provider mock·timeout·tenant 격리·감사 저장 계약을 고정하고, VAI-3~6 각 PR에서 S-90~S-100을 단계적으로 자동화한다. UI Preview 승인 전 코드 테스트를 만들지 않는다.
...
- **Technical_Specs**: [VAT AI Tax Treatment Completion Contract](../03_Technical_Specs/44_VAT_AI_TAX_TREATMENT_COMPLETION_CONTRACT.md) - JC-035 VAI-0~6 완료선
