...
import type { CompanyScheduleItem } from '@/lib/tax-calendar'
...
export type SebiseoWorkspaceProps = {
  ...
  readonly scheduleItems: readonly CompanyScheduleItem[]
  ...
  return (
    <div className="flex min-h-[calc(100dvh-3.5rem)] flex-1 flex-col bg-company-bg text-foreground md:min-h-screen">
      ...
      <div className="mx-auto w-full max-w-[768px] px-6 pb-4">
        <ReferenceTaxScheduleRows items={scheduleItems} />
      </div>
 ...
 * 일정 하나를 한 줄로 표시한다.
 */
function ReferenceTaxScheduleRows({
  items,
}: {
  readonly items: readonly CompanyScheduleItem[]
}) {
  if (items.length === 0) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-company-border bg-company-surface px-4 py-3">
        <span className="shrink-0 text-[12px] font-semibold text-company-fg-subtle">세무 일정</span>
        <span className="text-[14px] text-company-fg-muted">이번 달 남은 일정이 없습니다</span>
      </div>
    )
  }
    ...
    <div className="space-y-2">
      {items.map((item) => (
        <Link
          ...
          aria-label={`세무 일정 ${item.dateLabel} ${item.title} ${formatScheduleDeadline(item.dDay)}`}
        >
          <span className="shrink-0 text-[12px] font-semibold text-company-fg-subtle">세무 일정</span>
          <span className="shrink-0 text-[15px] font-bold tracking-tight text-foreground">{item.dateLabel}</span>
          <span className="min-w-0 flex-1 truncate text-[14px] text-foreground">{item.title}</span>
          <span className="shrink-0 text-[13px] text-company-fg-muted">{formatScheduleDeadline(item.dDay)}</span>
        </Link>
      ))}
    </div>
  )
...
function formatScheduleDeadline(dDay: number): string {
