...
- **JC-035 자동화 계획**: VAI-3a에서 Zod·deterministic 규칙·이전 확정 패턴·read model을, VAI-3b에서 provider mock·timeout/quota/invalid schema fallback·PII·batch/소비자 격리를 자동화했다. VAI-4~6은 mutation·감사·consensus·gate를 단계적으로 자동화한다.
