...
`blocked`로 두지 않는다.** Path 2는 Path 1a 베타 테스트 이후 검토한다. Path 3 인증·암호화
파일은 현재 제품 범위에서 제외한다.
...
| **2** | 세무회계사무소 연결 (자료기와) | **handoff ZIP 패키지** Export → JARYO-GIWA에서 검토 | 수임 사무소 (검정 SW) | JC-034 | **Path 1a 베타 이후** |
...
로 이어준다. Path 2는 Path 1a 베타 이후이며, 암호화 Path 3은 현재 범위 밖이다.
...
Path 1a 베타 이후에 Path 2를 검토한다. 암호화 Path 3은 현재 제품 범위에서 제외한다.
...
| 2 | **후순위** — Path 1a 베타 안정 후 | JC-034 ZIP (문서 완료, **구현 착수 보류**) |
