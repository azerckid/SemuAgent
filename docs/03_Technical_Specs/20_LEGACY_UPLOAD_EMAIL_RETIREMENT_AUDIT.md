...
- **Slice 2b 이후**: `lib/email/missing-request`, pipeline 내부 보충요청 초안 side effect, transaction purpose request의 `outbound_email` FK를 내부 self-use 방식으로 재설계하거나 제거한다.
