...
import type { PayrollAdaptiveCommonEngineResult } from '@/lib/payroll/adaptive-structuring-common-engine'
import type { PayrollAdaptiveStructuringEligibility } from '@/lib/payroll/adaptive-structuring-eligibility'
import type { PayrollAdaptiveStructuringProposalResponse } from '@/lib/payroll/adaptive-structuring-proposal-schema'
...
const STATUS_LABEL: Record<PayrollAdaptiveStructuringProposalResponse['status'], string> = {
...
function ProposalResult({ proposal }: { proposal: PayrollAdaptiveStructuringProposalResponse }) {
  return (
...
function EnginePreview({ enginePreview }: { enginePreview: PayrollAdaptiveCommonEngineResult }) {
  return (
    <div className="space-y-3 rounded-lg border border-dashed p-3">
      <div className="flex items-center gap-2">
        <Badge variant={enginePreview.matched ? 'outline' : 'destructive'}>
          {enginePreview.matched ? '엔진 재실행 검증 통과' : '엔진 재실행 검증 실패'}
        </Badge>
      ...
      {enginePreview.matched && (
        <>
          <p className="text-xs text-muted-foreground">
            ...
            {enginePreview.blockedRowCount > 0 && ` · 식별자/기간 누락으로 제외된 row ${enginePreview.blockedRowCount}개`}
          </p>
...
export function PayrollAdaptiveStructuringButton({
  sessionId,
  eligibility,
  candidateFiles,
}: {
  ...
  const [proposal, setProposal] = useState<PayrollAdaptiveStructuringProposalResponse | null>(null)
  const [enginePreview, setEnginePreview] = useState<PayrollAdaptiveCommonEngineResult | null>(null)
  const [registeredModelId, setRegisteredModelId] = useState<string | null>(null)

  const handlePropose = async () => {
    setLoading(true)
    const response = await fetch(`/api/sessions/${sessionId}/payroll/adaptive/propose`, {
      method: 'POST',
    })
    ...
    setProposal(data.proposal as PayrollAdaptiveStructuringProposalResponse)
    setEnginePreview((data.enginePreview as PayrollAdaptiveCommonEngineResult | null) ?? null)
    setRegisteredModelId(null)
  }

  const handleRegister = async () => {
    setRegistering(true)
    const response = await fetch('/api/payroll/adaptive-models', {
      method: 'POST',
    ...
    <Sheet onOpenChange={(open) => {
      ...
      <SheetContent side="right">
        <SheetHeader>
          ...
          <SheetDescription>
            제안일 뿐 적용이 아닙니다. 승인 전까지 급여 row, 결과 엑셀, 임금명세서, 메일 상태는 바뀌지 않습니다.
          </SheetDescription>
        </SheetHeader>
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
          {eligibility.reason}
        ...
        <div className="space-y-2">
          ...
          {candidateFiles.length === 0 ? (
            <p className="text-sm text-muted-foreground">후보 파일 정보를 찾을 수 없습니다.</p>
          ) : (
            <ul className="space-y-1">
              {candidateFiles.map((file) => (
                <li key={file.id} className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
        ...
        {proposal?.status === 'proposal_ready' && enginePreview?.matched && (
          registeredModelId ? (
            <div className="space-y-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
              ...
              <Link
                href="/dashboard/payroll/adaptive-models"
                className={buttonVariants({ variant: 'outline', size: 'sm' })}
              >
                구조화 모델 관리로 이동
              </Link>
