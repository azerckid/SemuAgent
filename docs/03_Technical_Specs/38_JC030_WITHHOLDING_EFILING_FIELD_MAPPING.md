...
```text
[Flow]
판정: 원천세 1a 양식 없음 → 1b(직접입력 안내) 구현 완료. Field Mapping Part A는 홈택스 화면·행/칸 위치표의 소스로 활용됨
Gate: 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) → 1a 파일 미해당, 1b로 제공
완료: 별지 제21호 A01 매핑 · NTS 공식 신고방법 감사 · 홈택스 메뉴 경로·기본정보·A01 ④⑤⑥ 위치와 live 값 연결
다음: 공식 양식이 확인되면 Part B(1a)로 승격
금지: 바이너리 레코드·fcrypt·암호화 파일을 Path 1 대체물로 구현
```

라벨 — **[확실]** law.go.kr 별지 제21호·JC-013 live 값·NTS 직접신고 흐름 · **[갭]** 홈택스 공식 비암호화 업로드 양식의 파일 필드 위치(1a 전용)
...
공식 조사 결과 원천세는 공식 비암호화 업로드 양식이 없어 **1b(직접입력 안내)로 구현 완료**됐다.
이 문서의 Part A 서식 정합은 1b 홈택스 입력 위치표의 소스이고, Part B·generator(1a)는 공식
양식이 확인될 때만 착수한다.
...
3. 양식이 없고 암호화 전자파일만 가능하면 1a로 승격하지 않고 1b(직접입력 정리)를 유지
