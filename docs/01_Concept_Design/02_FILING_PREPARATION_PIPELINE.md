...
- 베타에서는 Path 1만 제공한다. Path 2(사무소 handoff)는 Path 1a 베타 이후이며, Path 3(인증·암호화)은 현재 제품 범위에서 제외한다.
...
- **Path 1 (현재·베타):** 양식이 있으면 SemuAgent가 공식 비암호화 업로드 파일을 만들고(1a), 없으면 확정 데이터를 `항목 = 값`으로 화면에 정리한다(1b). 사용자가 값을 확인한 뒤 홈택스에서 직접 업로드하거나 직접 입력·제출한다. 양식이 없다는 이유로 세목을 `blocked`로 두지 않는다. (**구현 현황:** 1a는 간이지급명세서만 구현. 원천세·부가세는 1b 대상으로 결정됐으나 **1b 값 정리 화면은 미구현**. 세목별 상태는 [Backlog JC-030](../04_Logic_Progress/00_BACKLOG.md).)
- **Path 2 (후순위):** Path 1a 베타 이후 기존 세무회계사무소로 넘길 ZIP handoff를 검토한다.
- **Path 3 (범위 밖):** fcrypt·적합성 검정·암호화 파일은 현재 제품에 포함하지 않는다.
- 홈택스 화면에 옮겨 적을 값을 정리해 보여주는 **직접입력 정리(1b)** 는 베타 범위다. 다만 홈택스 메뉴·입력칸 위치를 단계별로 안내하는 가이드는 범위가 아니다.
    ...
    자료수집 -> 기장검토/자료대조원장 -> 확정 거래원장
                             ...
                             └─ 지방소득세(본세 종속)
                                          ...
                                          -> Path 1: 양식 업로드(1a) 또는 직접입력 정리(1b) -> 사업자 직접 제출
                                             (Path 2는 Path 1a 베타 이후, 암호화 Path 3은 범위 밖)
...
- Path 2 handoff 패키지(JC-034)는 Path 1a 베타 이후 검토한다.
- Path 3 인증·암호화 파일은 현재 제품 범위 밖이며 UI·generator·가이드에서 제공하지 않는다.
- 직접입력용 값 정리(1b)는 Path 1 완료 경로다. 다만 홈택스 메뉴·입력칸 위치 단계별 안내는 범위에서 제외한다.
