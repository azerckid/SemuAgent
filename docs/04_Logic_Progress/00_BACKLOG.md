...
- Status: `todo` — VAI-0·VAI-1 완료, VAI-2 규칙·저장 계약 작성 완료/승인 대기
...
- Implementation Preconditions:
  ...
  - [ ] VAI-2 Rule Matrix·Pre-Code Brief 프로젝트 오너 승인
- Acceptance Criteria:
...
- Document Sync Check: 2026-07-11 VAI-1 Preview 승인 완료. VAI-2 규칙 매트릭스와 Pre-Code Brief를 작성해 `홈택스 실제값`과 `자동채움 예상`, 확인·수정 행동 코드, VAI-3 read-only/VAI-4 mutation 경계를 고정했다. **구현 완료 아님.** VAI-2 승인 뒤 VAI-3a로 이동하며, JC-030 부가세 양식 업로드 Stage A는 병렬 외부 확인 대기로 유지한다.
