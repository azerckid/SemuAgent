...
완료: 근로소득 간이지급명세서 — 양식 채움 확인, 비암호화 파일 후보, 검증, 업로드 안내
현재: JC-030 Path 1 세목 확대 — 원천세 Slice 1b W0 비암호화 업로드 양식 확인
다음: 공식 비암호화 업로드 양식·수용 경로 확인 → 매핑/Brief 확정 → 파일 생성 → 실제 업로드 검증
...
| 1 | **원천세 신고서** | JC-012·JC-013 | 서식 매핑·UI·Slice 1a 완료, 비암호화 업로드 양식 미확인 | 실제 업로드 파일 생성 없음 | **현재 W0, 외부 확인 대기** |
...
|:---|:---|:---|
| **W0** | 공식 비암호화 업로드 양식·수용 경로 확인 | 공식 파일, 버전·적용일, 홈택스 업로드 메뉴와 직접 수용 여부를 기록 |
| **W1** | Field Mapping Part B·Brief 확정 | 모든 A01 v1 필드와 validation이 공식 양식 위치에 매핑되고 사용자 승인 완료 |
| **W2** | 동일 read model 기반 Preview·generator | 화면 값과 생성 양식이 동일하고 payroll 미마감·needs-review·합계 불일치 시 차단 |
| **W3** | generate API·download UI | tenant/client/월 scope, 서버 미보관, 정확한 content type/filename, 오류 표시 |
| **W4** | 파일·브라우저·홈택스 검증 | 결정론 테스트, 전체 회귀, 브라우저 다운로드, 실제 비암호화 업로드 검증 기록 통과 |
| **W5** | 문서 closeout | 원천세 §2.1 체크 완료, Backlog·QA·Audit 동기화 |

**다음 코드 작업은 W0과 W1이 끝난 뒤의 W2다.** W0에서 홈택스가 직접
수용하는 비암호화 업로드 양식을 확인하지 못하면 원천세는 blocked로 두고 W2를
작성하지 않는다. 바이너리 레이아웃·암호화 포맷을 대체 경로로 구현하지 않는다.
