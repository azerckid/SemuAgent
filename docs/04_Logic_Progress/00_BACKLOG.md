...
| JC-030 | todo | 전자신고 검증 및 파일 생성 (Validation / Path 1a·1b) | `lib/efiling-*`, JC-024·013 | **최우선 — Path 1 세목 확대.** 자료대조 Phase 2 완료. **간이지급명세서·원천세·부가세는 공식 양식 미확인 → Path 1b 직접입력 정리 화면 구현·검증 완료.** 간이지급의 과거 고정길이 후보 생성 UI는 제거했다. 다음은 지방소득세 특별징수 Path 1a다. 위택스 공식 매뉴얼에서 `B070101-02.xlsx` 엑셀파일신고를 확인했고, 로그인 원본 입수·구조 고정 전에는 generator를 만들지 않는다. Path 3 암호화·회계파일은 범위 밖. [Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md) · [Local Income Stage A Audit](../03_Technical_Specs/54_JC030_LOCAL_INCOME_TAX_UPLOAD_TEMPLATE_ACQUISITION.md) |
