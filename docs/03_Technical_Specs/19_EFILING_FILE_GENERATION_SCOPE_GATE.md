...
| 근로소득 지급명세서 | JC-024 월 확정 급여·직원 명부 + Stage B 현행 제24호서식 매핑 | Stage C 홈택스 정본, Stage D 승인 Preview, Stage E read model·상태·화면 상태 계약 완료 | **Path 1b 대상 · Stage F runtime 다음** |
...
준비한다고 결정했다. Stage D Preview 오너 승인과 Stage E Pre-Code를 완료했고,
다음은 신규 DB 없이 기존 급여·직원 데이터를 연결하는 Stage F runtime·QA다.
...
2. Output feeds Path 1b read-only screen. Path 2 JC-034 ZIP는 Path 1 세목 확대 안정 후 재사용.
