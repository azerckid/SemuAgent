...
```text
[Flow]
현재: JC-030 Slice 0a 완료 — 레이아웃 입수 경로 확정(간이지급 전용 문서)
Gate: 부분 통과 (PII·입수 경로 닫힘, 필드 매핑·Pre-Code Brief 미착수)
완료: Scope Gate §5.2 PII, [Layout Acquisition](./28_JC030_SIMPLIFIED_WAGE_EFILING_LAYOUT_ACQUISITION.md)
다음: UI Preview 또는 Pre-Code Brief(최신 HWP 다운로드 후)
필요 확인: 적합성 검정 요건, 구현 직전 정오표
...
- CTA: `식별정보 입력` → `검증 결과 보기` → `파일 다운로드` → `홈택스 파일변환신고 안내`
...
- [ ] 세목별 홈택스 전자신고 파일 **필드 레이아웃**을 Pre-Code Brief에 반영(HWP 다운로드 후)
- [ ] 확정 데이터 → 전자신고 파일 필드 매핑 — layout 입수 후 Pre-Code Brief
- [ ] 파일 형식·정합성 검증 규칙 — layout 입수 후
- [ ] 파일변환신고 적합성 검정 요건 — 0a·국세청 문의 병행 가능
